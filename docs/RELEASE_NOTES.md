...
- 候选包沿用仓库公共版本 `0.6.73`，不因连续开发阶段随意升版本；插件、Worker、Core、Contracts 使用同一构建身份 `0.6.73+248d28eff8c595516a803f8db356952cef54c166`。
- 候选 `.pext`/`.zip` 已由 `scripts/package.ps1` 从生产源提交 `248d28e` 在隔离输出中生成，Worker 为 self-contained `win-x64`，两个包均为 `43,837,966` 字节，SHA-256 均为 `628F34B01C478CD30A26260650703C8B77F558561E103D78B24C8A390949DDD3`；本包包含滚动锚点、浅色主题背景/云端筛选文字、首页活动内容、云端整卡导航、顶部复合按钮和活动游戏选择器视觉内容模板防护，以及对应的 WPF 回归断言。
