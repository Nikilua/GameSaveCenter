...
- 候选包沿用仓库公共版本 `0.6.73`，不因连续开发阶段随意升版本；插件、Worker、Core、Contracts 使用同一构建身份 `0.6.73+1fdd15eedfa64bb34292b85cb0e4d14bbfa9dd81`。
- 候选 `.pext`/`.zip` 已由 `scripts/package.ps1` 在隔离输出中生成，Worker 为 self-contained `win-x64`，两个包均为 `43,837,799` 字节，SHA-256 均为 `FD91FB0E0B12ABA2A73F29F76F1E3F90255D6FA4D30798EBA2FEFB53FAD120F9`；本包包含滚动锚点、浅色主题背景/云端筛选文字以及首页活动内容和云端整卡导航修复。
