...
- 候选包沿用仓库公共版本 `0.6.73`，不因连续开发阶段随意升版本；插件、Worker、Core、Contracts 使用同一构建身份 `0.6.73+8657654310d99e349c120f3bb0484b65ac9f4dc2`。
- 候选 `.pext`/`.zip` 已由 `scripts/package.ps1` 从当前 HEAD `8657654` 在隔离输出中生成，Worker 为 self-contained `win-x64`，两个包均为 `43,837,912` 字节，SHA-256 均为 `E136B5C6465A5A8933C72B0F9707CFF64D91A02D35115B9A007BF6A749B62AB0`；本包包含滚动锚点、浅色主题背景/云端筛选文字、首页活动内容、云端整卡导航、顶部复合按钮和活动游戏选择器视觉内容模板防护，以及对应的 WPF 回归测试覆盖。
