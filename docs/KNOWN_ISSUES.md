...
- **修复**：仅对媒体收件箱使用 `VirtualizingPanel.ScrollUnit=Item`、关闭列虚拟化、为内层 `ScrollViewer` 指定主题表面背景并裁剪到表格边界；不改变其他列表和业务命令。
- **回归**：在不同数据量、拖动到顶部/底部、快速滚轮、窗口缩放和 125%/150% DPI 下确认没有白色空框、空行或页面级滚动条。
