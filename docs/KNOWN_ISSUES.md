...
2026-09-03 GSC-132：用户确认目标游戏在行信息中显示“已安装”，但切换“已安装”后搜索不到；根因是 `SelectionChanged` 同时承载了用户选择和 WPF 初始化/集合刷新通知，程序化通知可能覆盖共享筛选状态。0.6.72 改为 `DropDownClosed` 唯一提交用户下拉选择，并新增精确回归测试；Release、全量自动化、Render QA 已通过，本机 Playnite 日志已确认加载 `0.6.72.0`。当前本机仅有 3 个样本游戏，不含“死亡空间”，用户目标机器需安装 0.6.72 并复核。

2026-09-04 GSC-133：用户实测 0.6.72 后仍然是“全部/已匹配/有备份可见，但已安装不可见”。进一步确认 Worker 的 `GameMatchInput` 为了保留既有匹配而排除安装状态，但目录同步又只在匹配输入变化时写入 SQLite，导致安装状态变化停留在旧缓存。0.6.73 已将描述持久化与匹配失效分离：安装状态、安装目录和启动动作会更新缓存，已有 Ludusavi 匹配保持不变；新增 Worker 精确回归测试，Release/全量测试/源码门禁通过，本机 Playnite 日志已确认加载 `0.6.73.0`。当前本机样本只有 3 个游戏，目标机器仍需复核。

2026-09-03 GSC-131：用户实机复核后确认 GSC-130 仍可能被 WPF 双向绑定竞态覆盖。两套选择器改为 OneWay 显示并由 `SelectionChanged` 唯一写回；Playnite 适配器补充有效本地 Play action/working directory 安装信号；发布版本提升到 0.6.71 以防止同版本旧 DLL 被继续复用。自动测试、打包、本机安装和 `extensions.log` 中的 `0.6.71.0` 加载证据已完成；若目标游戏仍显示未安装，应继续检查 Playnite 条目的 Play action 和安装目录，而不是仅凭 Ludusavi 匹配状态判断。当前样本未包含用户截图中的“死亡空间”，真实目标游戏仍需复核。

2026-09-01 自动审计收口：非宿主依赖项已完成开发并通过自动化验证。整库备份现在有 SQLite 主任务、逐游戏进度、Worker 启动恢复和任务中心重试；大库匹配不再遗失首批以外的待匹配项；媒体签名缓存带采样变更检测和容量/时间清理，媒体云端失败后重复同步也会重新尝试；媒体收件箱切换/刷新会保留最新模式和用户选择，Worker 离线时显示明确状态；详情刷新不会覆盖同一存档/媒体条目的未保存编辑草稿，游戏策略草稿也会跨快照保留，保存期间切换游戏不会串写策略基线或成功提示，游戏摘要会随同一条目刷新通知更新；异步保存/比较/预览/校验返回后只更新原始上下文，切换游戏不会污染当前列表、摘要或输入；Playnite 退出阶段的后台回调、任务长轮询和目录同步以及公共 IPC 入口已受生命周期取消保护；Worker 会话存档路径快照、游戏会话自动化和自动修改器审计写入也会跟随 ApplicationStopping，不在 SQLite 释放后继续扫描、备份、媒体同步、启动修改器或写停机期审计；自动修改器审计即使遇到停机期存储异常也不会从脱离任务冒泡；FLiNG 目录/版本解析已支持相对链接、HTML 解码和安全域名边界，版本读取也已加入逐项参数和旧响应隔离，下载进度写入已改为可等待且按百分比节流；原生确认框兜底也统一回到 Playnite UI Dispatcher，避免后台游戏事件直接调用宿主对话 API；初始同步取消会容忍后台任务已先行释放令牌源的卸载竞态；IPC 长连接读取不再为每次读取遗留无限取消等待对象；生产标题栏在 720 DIP 仍能完整显示所有操作按钮；剪贴板重试不再同步阻塞 UI 线程。真实 Playnite、DPI、宿主主题/高对比度、Worker 重启日志和实际大库滚动仍需手工验证。

2026-09-02 增补收口：媒体收件箱的旧代际加载现在在每页请求前后检查模式/生命周期代际；页面卸载或切换模式后不再继续发起剩余分页，也不再将旧结果写回 UI。当前正在执行的单个 IPC 请求仍受客户端既有超时约束，真实宿主快速切换与卸载仍需手工观察。

2026-09-03 功能增强：游戏级云端状态现在同时汇总存档上传和已归类媒体上传；媒体失败、等待重试或待上传时，不再被存档的“已上传”状态掩盖。该状态仍是本地 SQLite 的汇总展示，真实 Rclone 远端校验继续按测试计划执行。

2026-09-03 功能增强：Playnite 游戏右键菜单新增“同步媒体”，可直接对一个或多个选中游戏执行现有媒体同步流程；媒体归档关闭时不会提交请求。真实 Playnite 菜单显示和多选操作仍需宿主复核。

2026-09-03 功能增强：任务中心新增“重试可恢复”批量操作，按游戏和任务类型去重后逐项执行既有安全重试，单项失败不会中断其余任务；真实宿主批量确认和长任务行为仍需复核。
...
2026-09-03 增补收口：游戏选择器在移除状态/排序静态首项后，继续补上两套共享 WPF 选择框的合法过期选中值同步。生产 Shell 与兼容 Dashboard 的加载/平台选项恢复现在以共享 ViewModel 为准，不会再出现显示“全部”但实际仍按“已安装”筛选的状态。自动测试、Release 打包和本机安装核对通过；UI Automation 未进入真实 Dashboard，第二台机器仍需手工确认死亡空间在“全部/已安装”中出现。
...
| GSC-113 | 页面命令或 Playnite 快捷操作跨多个 await 后绕过专用守卫，在退出阶段继续提交 Worker 命令 | 源码已修复，待 Playnite 生命周期回归 | 公共 `RequestAsync` 在生命周期取消后返回取消任务；退出中不再创建新的 Named Pipe 请求或显示操作失败通知 |
| GSC-114 | FLiNG 下载每个分块都异步写任务进度，可能造成高频 SQLite 写入和未观察异常 | 源码已修复，待 FLiNG 下载回归 | 进度回调可等待且按百分比节流；大文件下载期间任务进度保持有序，数据库/Worker 异常进入当前任务失败处理 |
| GSC-115 | Worker 退出时会话存档路径快照仍使用 `CancellationToken.None`，可能在存储释放后继续扫描或回写 | 源码已修复，待 Worker 生命周期回归 | 快照任务绑定 `ApplicationStopping`；Worker 停止期间取消扫描，不再向已关闭存储提交结果 |
| GSC-116 | 游戏会话自动化使用 `CancellationToken.None`，Worker 退出时备份、媒体同步或修改器启动可能继续运行 | 源码已修复，待 Worker 生命周期回归 | 自动修改器、退出备份/媒体同步、定时备份/媒体同步绑定 `ApplicationStopping`；正常停机取消不记录业务失败 |
| GSC-117 | 延迟自动修改器启动的审计写入使用 `CancellationToken.None`，停机期间可能继续访问已释放存储 | 源码已修复，待 Worker 生命周期回归 | 跳过、成功和失败审计均使用延迟任务令牌；Worker 停止时取消后不再补写审计 |
| GSC-118 | 自动修改器启动失败后的审计补写异常未隔离，存储释放竞态可能让脱离任务产生未观察异常 | 源码已修复，待 Worker 生命周期回归 | 自动启动审计采用 best-effort；停机/存储异常只记录 Debug，不从后台任务继续冒泡 |
| GSC-119 | 游戏停止/快捷操作从后台续体进入原生确认框时可能跨线程调用 Playnite 对话 API | 源码已修复，待 Playnite 生命周期回归 | 原生确认兜底也通过统一 UI Dispatcher 调用；宿主关闭或调度失败时按未确认处理，不执行危险动作 |
| GSC-120 | 页面卸载取消初始同步时，后台任务可能先释放令牌源，导致卸载线程 `Cancel()` 抛出 `ObjectDisposedException` | 源码已修复，待 Playnite 生命周期回归 | 初始同步取消捕获释放竞态；页面快速打开/关闭不应产生未处理异常 |
| GSC-121 | IPC 长连接每次读取都会创建一个直到监听器取消才结束的无限 `Task.Delay`，持续任务事件可能累积取消注册和等待对象 | 源码已修复，待长时间任务通知回归 | 读取完成后取消注册立即释放；长时间事件连接的对象/令牌回调不随消息数持续增长 |
| GSC-122 | 媒体收件箱切换模式或卸载页面后，旧加载仍可能继续请求剩余分页并回写缓存 | 源码已修复，待页面切换/卸载回归 | 每页 IPC 前后检查加载代际；切换或卸载后不再继续分页，也不将旧结果应用到当前 UI |
| GSC-123 | 另一台电脑 Steam 已安装游戏，但 GameSaveCenter 搜索不到 | 源码已修复，待第二台 Playnite 回归 | Dashboard 打开后同步 Playnite 完整目录描述；Worker 先持久化空/旧缓存缺失的游戏，再后台节流匹配；实际安装目录存在时不被“已安装”筛选隐藏 |
| GSC-125 | 游戏级云端状态只反映存档上传，已归类媒体的失败/待上传状态可能被显示为已上传或未启用 | 源码已修复，待 Rclone/媒体回归 | Dashboard 一次聚合存档与已归类媒体状态，按失败、等待重试、待上传、已上传优先级展示 |
...
- **根因**：Dashboard 未打开时，游戏停止和快捷操作可能在后台续体调用 `ConfirmAsync`；原生确认兜底此前直接调用 `PlayniteApi.Dialogs.ShowMessage`，没有经过宿主 UI Dispatcher。
- **修复**：原生 `MessageBox` 结果先在 UI Dispatcher 内获取，复用通知和嵌入式确认的关闭态/异常隔离；无法安全调度时返回 `false`，危险动作不会继续执行。
- **回归**：源码定向测试、Release 构建和全量自动化通过；隔离 Playnite 中在 Dashboard 未打开时触发游戏停止确认、关闭宿主竞态，确认对话框只在 UI 线程显示，关闭期间不出现 Dispatcher 或未处理异常。

## GSC-120：初始同步取消的令牌源释放竞态

- **状态**：源码已修复，待隔离 Playnite 生命周期回归。
- **根因**：初始同步/大型目录缓存重试任务在 `finally` 中拥有 `CancellationTokenSource` 的释放权；页面卸载在交换字段后调用 `Cancel()` 时，后台任务可能刚好完成并先释放同一对象。
- **修复**：卸载取消保留后台任务负责释放的设计，同时捕获 `ObjectDisposedException`。该异常只代表任务已完成且没有剩余工作，不改变正常取消和代际失效逻辑。
- **回归**：定向 Playnite 源码测试、Release 构建和全量自动化通过；隔离 Playnite 中快速打开/关闭 Dashboard、Worker 启动与缓存重试期间卸载，确认不出现未处理异常。

## GSC-121：IPC 长连接读取器的取消等待对象累积

- **状态**：源码已修复，待长时间任务通知回归。
- **根因**：`BoundedIpcLineReader` 原先用无限 `Task.Delay` 与底层 `StreamReader.ReadAsync` 竞争；读取先完成时，Delay 及其取消注册仍保留到整个监听器令牌取消。任务事件消息持续到达时，会产生与读取次数相关的慢性对象和令牌回调积累。
- **修复**：改用 `TaskCompletionSource` 和一次性 `CancellationToken.Register`，读取完成或取消竞争结束后释放注册；不改变 IPC 行边界、超大消息丢弃和 4 MiB 上限。
- **回归**：阻塞读取取消与可释放注册测试、Release 构建、全量自动化和源码门禁通过；隔离 Playnite 中保持 Dashboard 任务事件连接长时间运行，确认内存/句柄/事件回调不会随消息数异常增长。

## GSC-122：媒体收件箱旧加载继续分页

- **状态**：源码已修复，待 Playnite 页面切换/卸载回归。
- **根因**：媒体收件箱原先只在所有分页完成后检查 `mediaInboxLoadGeneration`；切换模式或卸载页面期间，当前页返回后仍会继续请求下一页，完成后还可能更新不再可见的缓存集合。
- **修复**：`LoadMediaInboxPagesAsync` 接收请求代际，在每页 IPC 发起前和返回后检查代际；旧代际立即放弃后续分页，调用方不再进入 UI 回写。单个已发出的 IPC 请求仍由现有客户端超时完成，不改变公共请求取消语义。
- **回归**：源码契约、Release 构建、Core/Worker/Playnite 自动化和源码/WPF 门禁通过；隔离 Playnite 中连续切换“待归类/已忽略”、关闭 Dashboard，确认旧加载不再继续分页或产生退出期 UI 异常。

## GSC-125：游戏级云端状态汇总媒体上传

- **状态**：源码已修复，待 Rclone/媒体同步回归。
- **根因**：Dashboard 原先只读取 `games.cloud_state`，该字段主要由存档上传维护；已归类媒体的 `Pending`、`Failed` 或 `RetryScheduled` 状态没有参与游戏卡片、游戏选择器和详情页的云端状态计算。
- **修复**：SQLite Dashboard 聚合在现有媒体统计查询中同时计算媒体云端状态，并与存档状态按“失败 → 等待重试 → 待上传 → 已上传”合并；仅 `Assigned` 媒体参与，收件箱/已忽略副本不会污染游戏状态。没有媒体且未上传存档时仍显示原有“未启用”。
- **回归**：新增 Worker 持久化回归，验证“存档已上传但媒体失败”显示失败，以及“只有已同步媒体”显示已上传；Release 构建、全量自动化和源码/WPF 门禁通过。真实 Rclone 断网恢复和媒体长传输仍需手工验证。

## GSC-126：Playnite 游戏菜单直接同步媒体

- **状态**：源码已补充，待真实 Playnite 菜单回归。
- **背景**：媒体中心和游戏会话自动化已有同步能力，但 Playnite 游戏库右键菜单只有“立即备份”，用户必须先打开 Dashboard 才能手动归档媒体。
- **实现**：新增“同步媒体”菜单项，沿用现有 `EnsureWorkerAsync`、设置同步、游戏描述写入和 `MessageTypes.SyncMedia`；支持 Playnite 当前选中的多个游戏，`UploadAfterSync` 跟随插件云端开关。媒体归档关闭时显示提示，不提交任务。
- **回归**：新增 `QuickActionSourceTests` 契约断言；Playnite `325/382`（57 跳过）和源码校验通过。真实右键菜单、多选行为和任务完成通知仍需宿主复核。

## GSC-127：任务中心批量安全重试

- **状态**：源码已补充，待真实 Playnite 任务中心回归。
- **背景**：任务中心原先只能针对当前选中的一条失败/取消任务执行安全重试；备份、媒体同步和云端短暂失败积累后，需要反复选择并操作。
- **实现**：新增“重试可恢复”按钮和二次确认。候选任务按游戏与任务类型去重，只保留最新记录；`BackupAll` 与 `MediaInbox` 各只执行一次。批量流程复用现有单任务重试分流，逐项捕获异常并继续，完成后刷新 Dashboard 并展示成功/失败汇总。
- **回归**：新增 TaskCenter 源码契约测试；Release 0 warning/0 error、Core `65/65`、Worker `234/234`、Playnite `326/383`（57 跳过）、源码校验和 WPF 静态审计通过。真实宿主确认框、批量任务执行和长任务期间关闭页面仍需手工验证。

## GSC-128：设备冲突和媒体筛选选择状态

- **状态**：源码已修复，待真实 Playnite 多设备/媒体页面回归。
