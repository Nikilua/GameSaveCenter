...
- [x] GSC-126（2026-09-03）：Playnite 游戏右键菜单新增“同步媒体”，支持当前选中一个或多个游戏；复用 `media.sync`、媒体归档开关和云端上传设置，媒体关闭时明确提示且不发起请求。新增快捷入口源码回归；Playnite `325/382`（57 跳过）通过，真实右键菜单仍需 Playnite 宿主复核。

- [x] GSC-125（2026-09-03）：增强游戏级云端状态汇总。Dashboard 的 SQLite 聚合现在同时读取存档上传与已归类媒体上传状态，按失败、等待重试、待上传、已上传优先级展示，避免媒体失败被存档“已上传”掩盖；收件箱/已忽略媒体不参与游戏状态。新增 Worker 持久化回归；真实 Rclone 断网恢复和长传输仍需人工观察。
...
- [x] STAB-020（2026-09-01）：修复 IPC 长连接读取器的取消等待对象累积。每次读取不再遗留一个等待整个监听器生命周期才结束的无限 `Task.Delay`，改用读取完成后立即释放的取消注册，避免任务事件长连接在长时间运行后持续增加对象和令牌回调。阻塞读取取消回归通过；Release 0 warning/0 error、Core `65/65`、Worker `232/232`、Playnite `321/378`（57 跳过）和源码门禁通过；真实宿主长时间任务通知仍需人工观察。
...
- [x] STAB-011（2026-09-01）：收口 Playnite 退出阶段的后台生命周期。应用停止时先取消插件生命周期、停止任务通知计时器，再停止本插件持有的 Worker；排队的 Fire-and-forget、游戏启动/停止事件、任务长轮询和目录同步在退出边界不再创建新 IPC、通知或 UI 回写，正在进行的同步等待使用生命周期令牌退出。Release 0 warning/0 error、Core `65/65`、Worker `226/226`、Playnite `319/376`（57 跳过）和源码门禁通过；真实宿主退出、Worker 重启及 Dispatcher 关闭仍需人工观察。

- [x] STAB-010（2026-09-01）：收口异步操作返回后的上下文保护。媒体重新归类、单项/批量媒体元数据、存档备份完成提示、校验/恢复准备、策略模板应用、备份比较/保留预览和进程映射输入均捕获原始游戏/条目/输入；请求返回后只有原上下文仍可见时才回写列表、摘要或清理编辑状态，避免切换期间的旧结果污染当前页面。Release 0 warning/0 error、Core `65/65`、Worker `226/226`、Playnite `318/375`（57 跳过）和源码门禁通过；真实宿主快速切换仍需人工观察。

- [x] STAB-009（2026-09-01）：收口详情编辑草稿与游戏摘要刷新一致性。存档/媒体详情重新绑定同一 `BackupId`/`MediaId` 时，备注、锁定和收藏编辑器按字段记录 dirty 状态，只同步用户尚未修改的字段；游戏策略编辑在快照刷新时保留未保存的 `Policy`，保存请求捕获稳定的游戏 ID、名称和策略副本，切换游戏后不会错写基线或提示；游戏选择器在同一条目 DTO 更新后补发 `SelectedGame` 通知，壳层摘要不再停留在旧状态。Release、全量测试、源码/WPF 门禁和 Render QA 已验证；真实宿主刷新期间输入保持仍需人工观察。

- [x] UI-337（2026-09-01）：收口媒体收件箱与详情刷新的一致性。待归类/已忽略视图切换现在保留最新请求，忙碌期间的切换会在当前任务结束后补加载；旧响应只更新缓存，不覆盖当前视图、选中媒体或目标游戏。刷新存档/当前游戏媒体时按 BackupId/MediaId 保留用户选择；媒体收件箱补齐 Worker 离线态并隐藏冲突空态。Release 0 warning/0 error、Core `65/65`、Worker `226/226`、Playnite `315/372`（57 跳过）、源码/WPF 门禁和双主题多尺寸/连续 Resize/Shell `render-qa OK` 通过；真实宿主快速切换、Worker 重启与 DPI 仍待人工观察。

- [x] UI-336（2026-09-01）：修复 FLiNG 可下载版本读取的逐项操作与异步竞态。列表行按钮传递真实目录项；版本响应通过目录 ID/代际号校验，忙碌期间的新选择在当前操作结束后自动排队读取，避免旧响应覆盖新选择。Release 0 warning/0 error、Core `65/65`、Worker `226/226`、Playnite `314/371`（57 跳过）、源码/WPF 门禁和双主题多尺寸 `render-qa OK` 通过；真实宿主快速切换仍待人工观察。
...
- [x] UI-201（2026-08-11）：TaskCenter 堆叠详情 Inspector 最小高度 96→160 DIP；`TaskGrid` 236–460 有限视口、内部滚动、虚拟化、搜索/筛选/计数/命令/Binding 未变。离屏 QA：1040×700/1280×720/1366×768 下 `TaskDetailScrollViewer` 160 DIP 且内部滚动，任务队列仍 350/360/384 DIP 高、8 行；1600×900/1920×1080 保持右栏 360 宽 Inspector。源码验证通过；真实 Playnite 宿主、主题、DPI、连续缩放仍需手工验收。

- [x] UI-202（2026-08-11）：Overview 在内容区 `<1200` DIP（1280×720、1366×768 常用窗口）改为堆叠单列页面流，右侧摘要/风险卡下移，主工作区恢复 Demo 式宽度；Hero/当前游戏同行阈值 760→700，1040×700 也并排显示。离屏 QA：1040/1280/1366 下 Hero 与当前游戏同行且无重叠（444+266 / 576+346 / 630+378），最近活动由页面 Auto 滚动可达；1600/1920 保持宽屏双列。`OverviewActivityList` 有限视口、Recycling、真实命令/Binding 不变；`render-qa.ps1` 阈值同步为 1200。源码验证通过；真实 Playnite 宿主、主题、DPI、连续缩放仍需手工验收。

- [x] UI-203（2026-08-11）：存档中心/修改器中心堆叠 Inspector 最小高度 96→160 DIP（历史、候选、已安装工具、可下载版本四处），主表/列表仍保持 236 DIP 最小视口、内部滚动和虚拟化。离屏 QA 扩展覆盖 SaveCenterView/TrainerCenterView（假数据）：1040/1280/1366 下四个堆叠 Inspector 均为 160 DIP 且内部滚动，主表 236 DIP 高、8 行；1600/1920 保持右栏 360 宽。源码验证通过；真实 Playnite 宿主、主题、DPI、连续缩放仍需手工验收。

- [x] QA-002（2026-08-11）：离屏渲染 QA 覆盖补齐全部工作区与设置页（Overview/Save/Trainer/Media/Maintenance/Task/Settings × 5 种常用逻辑窗口）。设置页通过 Application.Resources 预置 `BaseTextBlockStyle` fallback 后可在无 Playnite 宿主环境解析；1040×700 下 Settings 四个 Tab 的 `SettingsScroller` 均 Auto 且超限可滚动。后续页面级改动可直接用 `scripts/render-qa.ps1` 做整体回归。

- [x] QA-003（2026-08-11）：`scripts/render-qa.ps1` 增加自动失败门禁：命名工作区主表/主列表视口 `<236` DIP（排除 `MaintenanceAuditLogGrid`/`OverviewActivityList`），或命名页面滚动面内容超限却 Hidden 时退出码 1。当前 7 页面 × 5 尺寸全绿，后续布局改动会直接被该门禁拦截。

- [x] QA-004（2026-08-11）：C 盘恢复可用空间后，重定向 `TEMP/TMP` 到 `.tmp/qa-temp` 并用隔离输出重跑完整测试：Core 13/13、Worker 23/23、Playnite 151/151 全部通过；Playnite 源码结构断言同步为 UI-201/202/203 新阈值。

- [x] QA-005（2026-08-11）：`scripts/dev-install-run.ps1 -Configuration Release -NoStart` 一键构建安装成功：解决方案 Release 0 警告/0 错误，Core/Worker/Playnite 全部通过，最新扩展已安装到本机 Playnite Extensions（未启动 Playnite）。真实宿主视觉验收待用户手工完成。

- [x] UI-204（2026-08-11）：新增共享 `UiFilterSelection.RestoreDefault`，任务中心 状态/游戏/类型 与 GamePicker 状态/平台/排序 下拉框在集合重建、页面加载、选择器打开时恢复默认“全部”，只在空选择时生效，不覆盖用户选择。render-qa 新增 ComboBox 空选择 PROBLEM 门禁并全绿；Playnite 151/151 通过。

- [x] UI-205（2026-08-11）：TaskCenter 动态筛选下拉框增加 200ms 短周期重试（最多 25 次）恢复默认选中，解决 Items 物化晚于 DataBind 导致游戏/类型仍为空；GamePicker 增加 Loaded 优先级二次恢复。render-qa 与 Playnite 152/152 通过。
...
- [x] PERF-003（2026-08-11）：GamePicker `RebuildPlatformOptions` 增加平台指纹（PlatformDisplay 顺序 + 数量），`Items` 未变化时跳过 `Distinct+OrderBy` 重建。render-qa 全绿；Playnite 152/152 通过。

- [x] PERF-004（2026-08-11）：GamePicker `GamePickerItem` 按 PlayniteId 缓存复用，快照只 `UpdateGame` 更新引用，减少大库对象分配并稳定选中对象身份；缓存上限 `max(1024, 2*游戏数+100)`。render-qa 全绿；Playnite 152/152 通过。
...
- [x] UI-185：存档中心候选路径页按 Demo 语义补齐“路径与校验”工作区：在既有候选表/Inspector 之前增加当前 Ludusavi 规则阅读卡，显示真实 `SelectedGame.LudusaviName`、匹配状态和健康/校验状态，并提供真实的立即扫描、重新校验、刷新详情入口；Dashboard 已提供的全局游戏上下文与操作栏不在 SaveCenter 重复渲染。`SaveCandidateLayout`、`SaveCandidateInspectorScrollViewer`、`SelectedCandidate`、候选接受/忽略/判断依据、DataGrid 虚拟化和窄屏有限堆叠均保留。新增结构回归测试；`python scripts/validate-source.py` 通过，隔离 Release 构建 0 错误（仅 NuGet 漏洞源不可访问警告），Playnite 145/145 通过。真实 Playnite 宿主、主题、DPI 和运行时连续缩放流畅性仍需手工验收。
...
- [x] UI-193（`477c332`）：修复 Maintenance 设备状态、异常审计和进程映射页在窄宽度/低高度堆叠 Inspector 时主表被压缩的问题。`FindingsGrid`、`MaintenanceDeviceGrid`、`MaintenanceAuditFindingsGrid`、`MaintenanceProcessGrid` 统一保留 236 DIP 最小表格视口；设备/审计 Inspector 的堆叠高度按实际布局扣除表格最低高度、上方 Auto 行或审计日志行和间距后的剩余空间计算，Inspector 内部滚动继续承载长内容；进程映射表补显式名称并接入共享表头主题加载。真实维护命令、Binding、选中项、DataGrid 虚拟化、审计日志有限高度与业务层未改变。新增/更新结构回归断言；`python scripts/validate-source.py` 通过，隔离 Release 构建 0 警告/0 错误，Playnite 149/149 通过。真实 Playnite 宿主、Light/Dark/Follow、高对比度、100%–200% DPI、窗口化截图和连续缩放流畅性仍需手工验收。

- [x] UI-194（`00157c5`）：修复页面级滚动条隐藏导致常用窗口下底部内容不可发现的问题。共享 `GscPageScrollViewer` 默认垂直滚动改为 `Auto`，设置页、存档策略、维护保留策略、侧栏导航和 Overview 辅助页面移除 `Hidden` 覆盖；Overview 仍由响应式代码在宽屏禁用不需要的内部列滚动、堆叠时启用 Auto，表格/列表内部虚拟化滚动保持。未修改命令、Binding、业务层和持久化；新增/更新滚动契约断言，`python scripts/validate-source.py` 通过，隔离 Release 构建 0 警告/0 错误，Playnite 149/149 通过。真实 Playnite 宿主、Light/Dark/Follow、高对比度、100%–200% DPI、窗口化截图和连续缩放流畅性仍需手工验收。

- [x] UI-195（`a97698d`）：补齐 Maintenance“设备状态 / 异常与审计 / 进程映射”三个 Tab 的页面级纵向滚动面，解决诊断/保留策略已有滚动而其他维护页底部仍可能被窗口边界裁掉的问题。新增三个命名 `GscPageScrollViewer`，表格由 `MaintenanceView.ApplyResponsiveLayout` 统一设置 236–460 DIP 有限 Height，页面滚动只负责到达下方 Inspector、审计条带和操作区，DataGrid 内部滚动、虚拟化、键盘和真实命令/Binding 保持；同步扩展源码大库门禁与结构回归断言。`python scripts/validate-source.py` 通过，隔离 Release 构建 0 警告/0 错误，Playnite 149/149 通过。真实 Playnite 宿主、Light/Dark/Follow、高对比度、100%–200% DPI、窗口化截图和连续缩放流畅性仍需手工验收。

- [x] UI-196（`6a622ea`）：补齐 TaskCenter 的页面级可达性契约。`TaskPageScrollSurface` 使用共享 `GscPageScrollViewer` 承载摘要、筛选、任务表和堆叠详情，`TaskGrid` 按窗口高度维持 236–460 DIP 有限视口，DataGrid 内部滚动/虚拟化与详情 Inspector 内部滚动不变；堆叠详情高度依据页面实际视口计算，避免外层 ScrollViewer 的无限内容测量把底部复制/重试/取消操作裁掉。同步更新大库源码门禁与 XAML 结构测试，保留所有真实搜索、筛选、计数、Binding、SelectedTask 和命令。`python scripts/validate-source.py` 通过，生产插件隔离 Release 构建 0 警告/0 错误，隔离测试输出 149/149 通过（构建阶段仅有 NU1900 漏洞源不可访问警告）。真实 Playnite 宿主、Light/Dark/Follow、高对比度、100%–200% DPI、窗口化截图和连续缩放流畅性仍需手工验收。

- [x] UI-197（`7e55be7`）：修复截图中三类常用窗口可见性问题。TaskCenter 动态游戏/类型筛选保留 `全部` 默认值，并在 ObservableCollection 重建清空 `SelectedItem` 的 WPF 绑定时序中由 `OnTaskFilterSelectionChanged` 延迟恢复空选择，保留用户已选项；生产隐式 ListBox 样式统一将内容和内部 ScrollViewer 锚定顶部，MediaGrid 的有限 236–460 DIP 视口、内部滚动、键盘访问和 Recycling 保持，避免少量媒体项目落到列表底部形成大块假空白；Overview 主列小于 720 DIP 时工作台按钮改为第二行横向 WrapPanel 自动换行，宽屏右侧摘要/风险列也启用有限 MaxHeight + Auto 滚动，底部“打开维护中心”可达。未改真实 Command、Binding、Worker、IPC、数据库、持久化或业务状态。`python scripts/validate-source.py` 通过，生产插件隔离 Release 构建 0 警告/0 错误，隔离测试 149/149 通过；测试构建仅有 NuGet 漏洞源审计的 NU1900。真实 Playnite 宿主、Light/Dark/Follow、高对比度、100%–200% DPI、窗口化截图和连续缩放流畅性仍需手工验收。
...
- [x] UI-184：首页 Overview 按 Demo `HomeView` 的层级收口：`OverviewHomeToolbar` 现在只承载“今日工作台”标题与全局批量命令；TODAY Hero 与真实当前游戏上下文进入同一 `OverviewHeroAndGameRow`，宽屏按 `1.25* / 14 / 0.75*` 双列呈现，继承 Demo 的“Hero + 当前游戏”关系，之后才是六项真实 `Snapshot.*` 指标和 `OverviewActivityList` 最近活动。实际主列小于 760 DIP 时 Hero/当前游戏自动堆叠；小于 720 DIP 时工作台按钮独占第二行，断点按 `OverviewPrimaryPanel.ActualWidth` 计算，避免把 Dashboard 总宽误当成 Overview 可用宽度。当前游戏的四个真实统计胶囊改为独立可换行行，避免窄双列中 Auto 列把卡片撑出边界；立即备份、刷新详情、查看需关注项、OverviewTasks/SelectedTask、Recycling 虚拟化与右侧风险滚动均保留，未新增业务逻辑或昂贵滚动特效。新增/更新结构测试锁定 Demo 层级和响应式契约；源码验证、Release 隔离构建 0 错误、Playnite 144 项测试通过，真实 Playnite 宿主、主题、DPI 和运行时流畅性仍需手工验收。
...
| 一键备份全部匹配游戏 | 🧪 | 提交即返回；Worker 后台逐游戏执行并经任务事件流实时反馈，长扫描不再阻塞面板 |
| 退出后自动备份 | 🧪 | Playnite 事件与进程侦测会话均可触发 |
| 默认 30 分钟定时备份 | 🧪 | 每游戏可配置，最低 5 分钟 |
| 基础成功/失败反馈 | 🧪 | 管理面板通过 Worker 任务事件流并以 SQLite 轮询作可靠回退，显示 Playnite 通知 |
...
| 云同步暂停语义 | 🧪 | `CloudTransferCoordinator` 通过全局闸门串行化 Rclone 传输，恢复前取得暂停租约；待真实 Rclone 并发/恢复回归 |
...
| 多设备冲突检测 | 🧪 | `DeviceStateService` 已写入/读取 sidecar、比较分叉并由维护页记录人工决策；待真实 Rclone/双设备回归 |
