...
> UI-202（2026-08-11）：Overview 的断点以 Dashboard 内容区逻辑 DIP 为准：`workspaceContentWidth < 1200` 时走堆叠单列页面流（`OverviewStackScrollSurface` 为唯一页面滚动面），右侧“今日概览/风险与提醒”下移；`>=1200` 才用主列+摘要列双列。Hero 与“当前游戏”同行的判断使用 `OverviewPrimaryPanel.ActualWidth < 700`，低于 700 才上下堆叠。这样 1040×700/1280×720/1366×768 下首页首屏都能看到 Hero 与“当前游戏”并排，且最近活动仍由页面 Auto 滚动可达；`OverviewActivityList` 的有限视口、ListBox Recycling、真实命令/Binding 和宽屏双列滚动分工保持不变。`tests/GameSaveCenter.RenderHarness` 的 Overview 模拟阈值必须与 Dashboard 同步（当前为 1200）。

> UI-203（2026-08-11）：存档中心/修改器中心所有堆叠 Inspector 的最小高度统一为 160 DIP（`Math.Max(160, Math.Min(...))`），不能退回 96 DIP 窄条：`SaveHistoryActionsScrollViewer`、`SaveCandidateInspectorScrollViewer`、`TrainerToolsSettingsScrollViewer`、`TrainerReleaseInfoScrollViewer`。`SaveHistoryGrid`/`SaveCandidateGrid`、修改器主表/可下载版本面板必须保留 236 DIP 最小视口、DataGrid/ListBox 内部滚动与虚拟化。离屏 QA 已扩展覆盖 SaveCenterView/TrainerCenterView，后续任何页面级响应式改动都应运行 `scripts/render-qa.ps1` 并把结果写入交接记录。

> QA-002（2026-08-11）：离屏渲染 QA 覆盖 Overview、Save、Trainer、Media、Maintenance、Task、Settings 七个页面 × 1040×700/1280×720/1366×768/1600×900/1920×1080 五种常用逻辑窗口。设置页单独构造依赖宿主 `BaseTextBlockStyle`，harness 必须保持 `Application.Resources["BaseTextBlockStyle"]` 的中性 fallback；Settings 的 `SettingsScroller` 为唯一页面滚动面，必须保持 Auto 且超限可滚动。

> QA-003（2026-08-11）：`scripts/render-qa.ps1` 是响应式回归的自动失败门禁。任何命名工作区 DataGrid/ListBox 有限视口 `<236` DIP（`MaintenanceAuditLogGrid` 审计条带与 `OverviewActivityList` 最近活动除外），或 `*ScrollSurface`/`SettingsScroller` 内容超限却 Hidden 时，render-qa 必须返回退出码 1。后续改动不得为了通过测试而把主表视口压到 236 以下或隐藏滚动条。

> QA-004（2026-08-11）：完整隔离测试基线与 UI-201/202/203 已对齐：Core 13/13、Worker 23/23、Playnite 151/151。Playnite 源码结构断言必须与生产阈值同步，当前约束为 `Math.Max(160, ...)`（Task/Save/Trainer 堆叠 Inspector）、`workspaceContentWidth < 1200`（Overview 单列流）、`primaryWidth < 700`（Hero/当前游戏同行）。运行测试时若 C 盘空间不足，先设置 `TEMP/TMP` 到仓库 `.tmp/qa-temp`。

> QA-005（2026-08-11）：`scripts/dev-install-run.ps1 -Configuration Release -NoStart` 是本机最新代码的标准安装路径，已成功安装到 `%APPDATA%\Playnite\Extensions\GameSaveCenter_66e9f2d7-67bb-43ef-b62a-b8e60734fcec`（版本 0.6.70）。用户可直接打开 Playnite 做真实宿主视觉验收；未运行真实 Playnite 前不得声称主题/DPI/键盘/连续缩放已验证。

> UI-204（2026-08-11）：筛选下拉框必须始终显示明确默认值，集合异步重建后不能留下空选择。`UiFilterSelection.RestoreDefault(ComboBox, string)` 是共享恢复路径，只在 `SelectedItem == null` 时把选中项恢复为逻辑默认（通常“全部”），绝不覆盖用户选择。TaskCenter 三个筛选框和 GamePicker 三个筛选框都必须接入该恢复路径（Loaded/DataContextChanged/CollectionChanged/面板打开）；render-qa 会把命名 ComboBox 空选择记为 `PROBLEM`。
...
> PERF-003（2026-08-11）：GamePicker `RebuildPlatformOptions` 必须保留平台指纹短路：`ComputePlatformFingerprint`（PlatformDisplay 顺序 + 数量）未变化时跳过平台选项重建。用户切换平台仍由 `PlatformFilter` 驱动 `RefreshNow()`，不得删除该短路。

> PERF-004（2026-08-11）：GamePicker 的 `GamePickerItem` 必须按 `PlayniteId` 缓存复用，`SetItems` 只调用 `UpdateGame` 更新内部 `GameStatusDto` 引用；缓存超过 `max(1024, 2*游戏数+100)` 才清空。禁止每次快照整库 `new GamePickerItem`，这既浪费大库分配又让选中引用漂移。
