...
> 2026-09-08 Q6-04 兼容边界修正进行中：旧 Worker 缺少构建身份、或任一侧为 `+unknown` 时，`WorkerLauncher` 按版本/协议继续兼容并记录“身份未验证”；只有两个已知身份不一致才拒绝复用。定向构建身份测试 `3/3` 已通过，隔离打包正负例仍待完成，不能把当前代码测试写成包发布链已验收。
