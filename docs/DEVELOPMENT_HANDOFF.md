...
> 2026-09-21 第三轮 R21-02 Dashboard 选框与策略控件续作已由 `6a8cae16` 收口并推送：复用游戏状态/平台/排序筛选、同步路径和当前游戏策略编辑器的既有 Binding 与命令，为三个 ComboBox、六个策略开关及保存动作补稳定 Automation 名称；`R21AutomationValueBehaviorTests 8/8`，相关进度/焦点/键盘/无障碍/生产壳层 `56/56`，实际 WPF peer 验证名称、选项切换、Toggle `Off/On` 和保存按钮名称。source-copy Release Playnite `net462` / Tests `net472` `0 errors / 0 warnings`，source/XAML/diff 通过，WPF 静态检查 `0/27/177`。真实 Playnite/package-host、Windows UIA/读屏、OS 输入、IME、物理 DPI/跨屏、呈现和宿主性能待验；Demo 原目录不可用，main 用户改动未碰、未合并。证据见 `design/reviews/ui-finesse-round3-20260915/evidence/R21-02-DASHBOARD-AUTOMATION-20260921.md`。下一可执行任务：继续盘点其他复合选择器和逐控件状态值负例，之后进入 R21-03 错误播报。
