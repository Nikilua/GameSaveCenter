...
| Disclosure / segmented / controls | 部分收口 | 生产已有共享 `Redesign.xaml` 控件资源；Trainer/Settings 使用分段或分类面板，业务折叠区保留真实状态。 | 对照 `LabDisclosure`、`LabSegmented`、按钮、输入框、ComboBox、开关和展开动画，避免页面局部再造第二套样式。 |
...
| 证据与交付 | 未完成 | Release 构建、测试、source/WPF 静态门禁、RenderHarness 双主题多尺寸及 UI 审计持续通过；每阶段独立中文提交并推送。 | 真实 Playnite 宿主逐页像素、DPI、主题、键盘、命中区域和真实操作证据仍未完全收口，不能以离屏截图代替。 |
...
| Trainer | `TrainerPage.xaml`：四项分段导航、工具列表/详情、导入确认、在线目录、发行版本。 | `Views/TrainerCenterView.xaml`：`TrainerSegmentTabs`、`PanelTools`、`PanelImport`、`PanelCatalog`、`PanelReleases`，真实工具编辑和目录/发行命令。 | 页面骨架已直接对应，生产虚拟化/滚动和紧凑详情入口保留。 | 核对四个面板的卡片层级、工具编辑复选框/按钮、导入确认与发行下载状态的可达性。 |
| Settings | `SettingsPage.xaml`：左侧分类栏、右侧单一内容区、设置卡片和控件。 | `Settings/GameSaveCenterSettingsView.xaml`：`SettingsSectionTabs`、五分类面板、`SettingsScroller`、真实验证/保存/导入导出。 | 信息架构已对应，设置主题资源已与 Shell 共用核心色板。 | 核对分类选中态、输入/开关/下拉/按钮样式、校验错误和低高度滚动；补齐真实宿主嵌入截图。 |
...
最近阶段：UI-269 `3294edf`（固定 Demo 核心主题配色关系）；前一阶段：UI-268 `f8d6b46`（补齐 Demo 标题字体阶关系）。
