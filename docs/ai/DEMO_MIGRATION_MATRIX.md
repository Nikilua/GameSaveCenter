...
| 证据与交付 | 未完成 | Release 构建、测试、source/WPF 静态门禁、RenderHarness 双主题多尺寸及 UI 审计持续通过；真实宿主审计已取得 Settings `EmbeddedPlaynite` 截图，但 Dashboard 仍未嵌入捕获。 | 真实 Playnite Dashboard 七页逐页像素、DPI、主题、键盘、命中区域和真实操作证据仍未完全收口；`ControlledDashboardCaptured=true` 不能替代 `EmbeddedDashboardCaptured=true`。 |
...
最近代码阶段：UI-271 `c6cb235`（统一 Demo 表格字阶）；最近宿主证据阶段：`artifacts/ui-host-audit-ui271`（Settings 嵌入证据，Dashboard 未捕获）。
