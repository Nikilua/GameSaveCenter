...
- 状态：代码与离线测试已完成，真实 Playnite 分页刷新、状态变化、故障注入和录屏待宿主验收。
- 修复：`MaintenanceCloudTransferResolver` 先合并快照和分页明细；`UpdatedUtc` 较新者胜出，同时间分页明细优先。摘要计数扣除被新明细解决的快照告警，并计入明细中新出现的告警。
