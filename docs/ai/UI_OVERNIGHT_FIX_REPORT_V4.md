...
- 折叠框：`GscDisclosureCard` header 的文字改为与左侧 chevron 图标垂直居中，避免“文字偏上/偏下”的错位感。
- 存档数字输入：`GscNumericFieldInput` 数字改为水平居中、内容垂直居中，输入框外尺寸不变；当前游戏与策略模板共 5 处输入同步应用。
- 回归：Playnite `255/255`、render-qa 全绿、v4 截图已重新生成。
