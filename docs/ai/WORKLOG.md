...
## 2026-09-23 Round3 R18-04 表格容器预算当前复核

- 在 D 盘 `D:\workplace\github\GameSaveCenter`、分支 `codex/ui-finesse-round2`、当前构建身份 `f175c57dda95f6eee9031f0363349189c4e47bb9` 复核已有 `64843642` 实现；未改生产代码，也未从 main 覆盖当前实现。
- 隔离 Release solution 构建 `0 errors/2` 条既有 `MediaCenterView.xaml.cs:706 CS8602` warning，XAML `24/24`。R18 专测 `1/1`；分页/锚点/几何/选择行为按类分别为 `6/6 + 10/10 + 3/3 + 4/4 = 23/23`。
- 当前 2k/10k/20k Task 均 `9` 个最大已实现容器、`7` 个视口/可见行，滚动最大 `60.790/30.387/38.595 ms`。Media UI 窗口保持 2,000 项；当前共享模板与 `1280×720 DIP` 视口为 14 行，最大容器/可见 `14/14`，滚动最大 `0.026/0.027/0.031 ms`。记录的逐类、逐样本结果见证据。
- R18 与媒体锚点 WPF testhost 关闭阶段出现 TextServicesHost `InvalidComObjectException` 调试输出；xUnit 测试通过且 dotnet 退出码为 0，根因未确认。仅为合成数据/隔离 STA/逻辑 DIP，不宣称真实 Playnite、物理屏幕或 ETW 性能通过；Demo 原目录不可用。

证据：[R18-04 当前表格容器复核](../design/reviews/ui-finesse-round3-20260915/evidence/R18-04-TABLE-CONTAINER-BUDGET-RECHECK-20260923.md)。下一项：按用户优先级复核和校正 R00/R01 当前证据身份与待验边界，再推进依赖已满足的 Q/R 小批。
...
- Worker `RetentionQuarantineRecoveryTests 5/5`；Playnite R17 合并 `12/12`，隔离账本源码/绑定 `2/2`；当前提交隔离 Release `0 errors/2 warnings`，均为 `MediaCenterView.xaml.cs:706` 既有 `CS8602`。`validate-source.py`、XAML `24/24`、diff、WPF `0/28/162` 通过。
- 已验证分页总量、原/隔离路径、完整 Tooltip、EntryId 定向恢复、移动中断恢复、已知隔离文件清理和冲突停止；只用合成/fake/隔离 SQLite，没有真实配置、存档、媒体、云端或诊断写入。真实宿主和呈现仍待验，Demo 原目录不可用。

证据：[R17-05 隔离账本入口定向复核](../design/reviews/ui-finesse-round3-20260915/evidence/R17-05-QUARANTINE-LEDGER-RECHECK-20260923.md)。下一可执行任务：`R17-06 存储分析导航`。
...
- Worker `HealthInspectionServiceTests 12/12`；Playnite R17 合并回归 `10/10`，其中 R17-03 `3/3`；当前提交隔离 Release `0 errors/2 warnings`，均为 `MediaCenterView.xaml.cs:706` 既有 `CS8602`。`validate-source.py`、XAML `24/24`、diff、WPF `0/28/162` 通过。
- 已验证游标恢复、范围/预算/延后摘要、游戏运行和操作锁门禁、候选先持久化、取消不伪装整库完成、失败分类、最近成功/完成和下轮计划；只用合成/fake/隔离 SQLite/目录，没有真实配置、存档、媒体、云端或诊断写入。真实宿主和呈现仍待验，Demo 原目录不可用。

证据：[R17-03 检查进度预算定向复核](../design/reviews/ui-finesse-round3-20260915/evidence/R17-03-INSPECTION-PROGRESS-BUDGET-RECHECK-20260923.md)。下一可执行任务：`R17-04 保留预览对比`。
...
- R16 行为 `4/4`、源码 `1/1`、既有 `PortableSettingsTests 10/10`，合计 `15/15`；当前提交隔离 Release `0 errors/2 warnings`，均为 `MediaCenterView.xaml.cs:706` 既有 `CS8602`。`validate-source.py`、XAML `24/24`、diff、WPF `0/28/162` 通过。
- 已验证预览不改草稿、确认后应用、未知字段、旧架构/坏值、异常回滚、凭据和设备身份边界；只用合成 JSON/detached settings/隔离目录，没有真实用户配置、存档、媒体、云端或诊断写入。真实宿主和最终呈现仍待验，Demo 原目录不可用。

证据：[R16-07 配置导入预览定向复核](../design/reviews/ui-finesse-round3-20260915/evidence/R16-07-SETTINGS-IMPORT-PREVIEW-RECHECK-20260923.md)。下一可执行任务：`R16-08 保存冲突处理`。

## 2026-09-23 Round3 R16-06 生效条件说明定向复核

- 本批在 D 盘 `D:\workplace\github\GameSaveCenter` 的 `codex/ui-finesse-round2`、`9cef273b` 上复用 `83e7c745` 的生效条件提示和 `EndEdit → NotifyVisualSettingsChanged → settings.update → WorkerOptions.Apply/SyncPlan` 链路，没有新增生产代码。
- `R16SettingsEffectSourceTests 2/2` 加 R16-05 路径回归 `6/6` 通过；当前提交隔离 Release `0 errors/2 warnings`，均为 `MediaCenterView.xaml.cs:706` 既有 `CS8602`。`validate-source.py`、XAML `24/24`、diff、WPF `0/28/162` 通过。
- 已确认源码测试只覆盖提示/链路/负例契约，没有把它写成真实 Playnite 保存后时序；未使用真实存档、媒体、云端或用户配置，Demo 原目录不可用。真实宿主呈现、Worker 生命周期、DPI/UIA/IME、ETW/宿主性能仍待验。

证据：[R16-06 生效条件说明定向复核](../design/reviews/ui-finesse-round3-20260915/evidence/R16-06-SETTINGS-EFFECT-CONDITIONS-RECHECK-20260923.md)。下一可执行任务：`R16-07 配置导入预览`。
...
- Core `BackupPolicyTemplateCatalogTests 5/5`；Playnite `R16PolicyDiffSourceTests + R16PolicyTemplateBatchSourceTests 2/2`。实际覆盖 13 字段差异、复制回退、取消不发 Worker 请求和未保存草稿禁用模板负例。
- Playnite `net462` 定向构建无错误，保留 `MediaCenterView.xaml.cs:706` 两条既有 `CS8602` warning；`validate-source.py`、XAML `24/24`、diff check 通过。R16-02 校正为“已满足，待环境验证”。
- 未验真实 Playnite 差异卡片/最终呈现、浅深主题、UIA/读屏/IME、DPI/跨屏、presented frame、ETW 或宿主性能；Demo 原目录不可用。

证据：[R16-02 策略差异预览定向复核](../design/reviews/ui-finesse-round3-20260915/evidence/R16-02-POLICY-DIFF-RECHECK-20260923.md)。下一可执行任务：`R16-03 模板应用范围`。
...
- `R22TimeDisplayBehaviorTests 30/30`、`R10RecentAccessBehaviorTests 2/2`；隔离 Release XAML `24/24`、Playnite `net462`、`0 error`，仅保留 `MediaCenterView.xaml.cs:706` 两条既有 `CS8602` warning。
- 证据：[R22-01 残余时间入口复核](../design/reviews/ui-finesse-round3-20260915/evidence/R22-01-RESIDUAL-ENTRY-RECHECK-20260923.md)。未验真实 Playnite/UIA/读屏/IME、DPI/跨屏、presented frame、ETW、宿主性能和 Demo 原目录；下一项为 R23-04 正常可枚举宿主会话，受阻则推进独立 Q/R。
...
- 覆盖 StageMessage 写入/查询/迁移、可读阶段、未知进度 `—`、失败/取消终态错误分离；XAML `24/24`、solution `0 error/2` 条既有 warning、Playnite `net462` 编译通过。
- 既有 `Task-1600x900` 仅作合成 offscreen 阶段列/未知阶段负例视觉证据；真实宿主阶段事件覆盖、DPI/UIA/IME、presented frame、ETW 和宿主性能未验。Demo 原目录不可用，下一项 R15-02。

证据：[R15-01 任务阶段可读定向复核](../design/reviews/ui-finesse-round3-20260915/evidence/R15-01-TARGETED-RECHECK-20260924.md)。
...
- 证据/账本提交 `6cb2e53c` 已由 `3801184d` 合并到 `main` 并推送；主分支 Release 复核同样为 `0 error / 2` 条既有 warning。
...
- 证据/账本提交 `1292b296` 已由 `6aed472b` 合并到 `main` 并推送；主分支 Release 复核同样为 `0 error / 2` 条既有 warning。
...
- 终审逐页记录空、错、加载、可读性、可达性和 Demo 对应/例外；工具/设置/壳层没有强行套用数据工作区 Empty presenter，历史截图不作为当前呈现通过。
- 仅同步报告、账本、记忆和日志，无生产代码修改；未启动真实 Playnite/package-host，不做真实存档/媒体/云端/诊断写入。下一可执行项为 `R23-04` 非空隔离宿主。
...
- 账本仍明确区分已满足、已实现待环境验证、部分满足、外部阻塞和不适用；没有用 R23-01 的交付审计替代 R02-06 宿主菜单阻塞或 R22-01 未完时间入口。
- 当前分支 clean 且与远端同为 `553b38a9`，未触碰 main 用户改动；R22-08 精确构建临时目录已清理。本阶段只同步证据、账本、记忆和日志。
- 证据：`docs/design/reviews/ui-finesse-round3-20260915/evidence/R23-01-GROUP-DELIVERY-20260922.md`。下一可执行项：`R23-02` 生产资源状态矩阵。
...
- `f1e568d0` 新增 `RetryTimingRelativeDisplay`、`RetryTimingFullDisplay`、`RetryTimingRawUtcDisplay`，详情正文显示“约多久后/可立即重试/无自动重试”，Tooltip/Automation HelpText 保留完整下次 UTC 证据；旧 `RetryTimingDisplay`、`NextAttemptLocal` 和队列状态合同保留。
- 合成 DTO 覆盖未来重试、已到期立即重试、无自动重试和未记录 UTC；`R22TimeDisplayBehaviorTests` `18/18`，云端/Health/Environment/时间/维护定向 `40/40`，R13 云端队列及相邻维护门禁均通过。联合回归 `35 passed / 3 failed / 0 skipped`（总计 38），3 条仍为旧 `SourceRule` fixture、分类 `SelectedIndex`、Sidebar 版本文本断言漂移。
- 代码提交 `f1e568d0` 已推送 `codex/ui-finesse-round2`；隔离 Release 构建 XAML `24/24`、Playnite/Tests `0 errors`，保留 2 条既有 `MediaCenterView.xaml.cs:671 CS8602`；`validate-source.py`、`git diff --check`、WPF 静态审计 `0 errors / 27 warnings / 177 info` 通过。只使用合成 DTO、fake/隔离测试宿主和隔离目录，未写真实存档、媒体、云端或外发诊断；Demo 原目录不可用，main 用户改动未碰、未合并。
- R22-01 仍“部分满足，待继续”。真实剪贴板、云端/Playnite/package-host、UIA/读屏、OS 输入/IME、DPI/跨屏、最终呈现、ETW、宿主性能未验；下一可执行任务为核对 Maintenance 动作摘要中 CloudTransfer 的 `LastAttemptDisplay`/`NextAttemptDisplay`，再盘点 Storage。
...
- `058ca8ff` 保留 `LastSyncDisplay` 的本地时间和“尚未同步”，新增相对/完整/原始 UTC 投影；镜像状态卡片显示相对时间，完整值进入 Tooltip/Automation HelpText。复制、校验、取消和“绝不删除镜像中的多余文件”语义保持。
- 最终 Release 构建身份为 `058ca8ff`：Playnite `net462`、Playnite Tests `net472` `0 errors`，主项目仍有 2 条既有 `MediaCenterView.xaml.cs:671 CS8602` warning；`R22TimeDisplayBehaviorTests 13/13`、`MaintenanceReportSourceTests | R17FindingTriageBehaviorTests 7/7`、`R06SortingBehaviorTests 4/4`、`R11HistoryTimeNavigationBehaviorTests 3/3`，定向合计 `27/27`。`validate-source.py`、XAML `24/24`、`git diff --check`、WPF `0/27/177` 通过。
- 本批只使用合成 DTO、fake/隔离 testhost 和隔离目录，未写真实存档、媒体、云端或外发诊断；Demo 原目录不可用，main 用户改动未碰未合并。R22-01 仍为“部分满足，待继续”，下一项盘点 Storage/RecentAccess 等其他仍直显旧本地时间的生产绑定。
...
- `R03LongPathTests`、`R11DiffListSearchBehaviorTests`、`R15TaskFailureCopyTests`、`R21SelectableTechnicalTextBehaviorTests` 合计 `13/13`：长路径、SaveCenter 完整路径、脱敏错误码/技术详情/任务 ID 和版本文本均通过实际 WPF 选择/复制相关行为，不是单纯源码字符串断言。
...
- R21-06 按“已满足，待环境验证”收口；下一可执行任务：`R21-07` 焦点可视回归。证据：`docs/design/reviews/ui-finesse-round3-20260915/evidence/R21-06-SELECTABLE-TECHNICAL-TEXT-20260921.md`。
...
- `R21AutomationValueBehaviorTests 21/21`；相关筛选 `35/35`。D 盘 source-copy 以 `GscBuildCommit=efb42b7b3dd6f259954cb3c8d170e97776428c9d` Release 构建 Playnite `net462` / Tests `net472`，`0 errors / 2` 条既有 `MediaCenterView.xaml.cs:671 CS8602` warning；source validation、XAML `24/24`、diff、WPF `0/27/162` 通过。
- 链接工作树 `_wpftmp.csproj` 仍受 `Access denied` 限制，未绕过；source-copy/build 已清理。只使用合成选项、fake/隔离 testhost；Demo 原目录不可用，main 用户改动未碰、未合并。三态不代表当前业务 Binding 会产生 Indeterminate，真实 Playnite host、UIA/读屏、呈现、DPI/IME、性能仍未验。
...
- `R21AutomationValueBehaviorTests 16/16`；相关回归 `64/64`。测试反射等待真实生产私有异步方法，未用字符串断言冒充行为。
- D 盘源码副本以 `GscBuildCommit=f7b664f1` Release 构建 Playnite `net462` / Tests `net472`，`0 errors / 2` 条既有 `MediaCenterView.xaml.cs:671 CS8602` warning；source validation、XAML `24/24`、diff、WPF `0/27/177` 通过。
- 链接 `_wpftmp` 仍受 `Access denied` 限制，未绕过；source-copy/build 已清理。只使用合成参数/fake/隔离 testhost；Demo 原目录不可用，main 用户改动未碰、未合并。
- 下一可执行任务：核对批量动作忙碌态/CanExecute 的真实绑定边界，再继续其他状态/值负例；R21-02 公共门禁完成后进入 R21-03 错误播报。真实宿主 UIA、呈现、DPI/IME、性能仍未验。
...
- 新增实际 WPF Peer 行为证据：名称、`Off → On → Off` 回切负例；`R21AutomationValueBehaviorTests 13/13`，相关回归 `61/61`。
- D 盘源码副本以 `GscBuildCommit=42f5744d` Release 构建 Playnite `net462` / Tests `net472`，`0 errors / 2` 条既有 `MediaCenterView.xaml.cs:671 CS8602` warning；source validation、XAML `24/24`、diff、WPF `0/27/177` 通过。首次 `working-tree` 身份不一致被测试夹具拒绝，统一提交身份后已重跑通过。
- 链接 `_wpftmp` 仍受 `Access denied` 限制，未绕过；source-copy/build 已清理。只使用合成/fake/隔离 testhost；Demo 原目录不可用，main 用户改动未碰、未合并。
- 下一可执行任务：继续盘点 MediaCenter 备注/元数据动作和其他逐控件状态/值负例；R21-02 公共门禁完成后进入 R21-03 错误播报。真实宿主 UIA、呈现、DPI/IME、性能仍未验。

## 2026-09-21 R21-02 SaveCenter 策略控件外置标签与状态（续作小批量）

- `e1fadaab` 已提交并推送：不改生产 XAML，复用 SaveCenter 四个策略开关、异常保护/策略模板选择器和锁定版本 CheckBox 的标签、Binding 与 Automation 名称，补实际 WPF peer 名称与状态/选值变化证据。
- `R21AutomationValueBehaviorTests 12/12`；相关进度/焦点/键盘/无障碍/生产壳层 `60/60`。新增测试实际创建四个 ToggleSwitch、两个 ComboBox 和一个 CheckBox peer，验证 Toggle/CheckBox `Off → On`、名称和两个选值切换。
...
- 链接工作树 WPF `_wpftmp.csproj` 写入仍遇 `Access denied`，没有提升或绕过；改用项目已有 D 盘源码副本流程，验证后清理 source-copy/build。只使用合成/fake/隔离 testhost；Demo 原目录不可用，main 用户改动未碰、未合并。证据：`evidence/R21-02-SAVECENTER-POLICY-PEER-20260921.md`。
...
- `R21AutomationValueBehaviorTests` `8/8`；相关进度/焦点/键盘/无障碍/生产壳层筛选 `56/56`。新增测试实际检查三个 Dashboard ComboBox、六个 ToggleSwitch 和保存按钮的 peer 名称，验证选项切换与 Toggle `Off→On`。
- source-copy Release Playnite `net462` / Tests `net472` `0 errors / 0 warnings`；`validate-source.py`、XAML `24/24`、diff 通过；WPF 静态检查 `0/27/177`，未见本批新增诊断。
- 仅使用生产 Dashboard XAML、合成 WPF peer、fake/隔离 testhost/source-copy；Demo 原目录不可用，main 用户改动未碰、未合并；本批临时 source-copy/build 已清理。证据：`evidence/R21-02-DASHBOARD-AUTOMATION-20260921.md`。
- 下一可执行任务：继续盘点其他复合选择器和逐控件状态值负例；公共门禁完成后进入 R21-03 错误播报。真实宿主 UIA、呈现、DPI/IME、性能仍未验。
...
- `R21AutomationValueBehaviorTests` `4/4`；相关 R21 键盘/焦点/无障碍/生产壳层回归 `35/35`。受控 WPF peer 实际覆盖 Toggle 名称与 `Off/On`、ComboBox 选中值，既有 glyph 与 ProgressBar RangeValue 证据继续通过。
- 隔离 source-copy Release Playnite `net462` / Tests `net472` 无错误；首次编译仅有 2 条既有 `MediaCenterView.xaml.cs:671 CS8602` warning，后续 no-restore `0/0`；`validate-source.py`、XAML `24/24`、diff 通过；WPF 静态检查 `0/27/177`。source-copy/build 已清理。
- 不把 R21-02 写成完成：TaskCenter DataGrid 进度条、Maintenance 远端备份进度、其余复合选择器和状态/值负例待继续。只使用合成控件、生产源码契约和隔离 testhost；Demo 原目录不可用，main 用户改动未碰、未合并。证据：`evidence/R21-02-SAVECENTER-AUTOMATION-20260921.md`。
- 下一可执行任务仍为 R21-02 剩余进度条名称和值与负例，之后进入 R21-03 错误播报。
...
- `FilterConditionSummary.StaleStateDetail` 统一 stale 文案；定向 `37 passed / 1 failed / 0 skipped / 38 total`，唯一失败为未修改的 `FailedTaskDetailsPutUserReasonBeforeCollapsedTechnicalDetails` 断言漂移。隔离 Release `0 errors / 2` 既有 warning；source、XAML `24/24`、diff 通过。提交 `c492bbc2`。
- 只用合成/fake/隔离 testhost 和目录，未写真实存档、媒体、云端或诊断；未宣称真实宿主、呈现、DPI/UIA/IME、ETW、宿主性能。Demo 原目录不可用，main 用户改动未碰。证据：`evidence/R20-05-STALE-UNDERSTANDABLE-20260921.md`。
- 下一可执行任务：从 Q/R 依赖账本选择依赖已满足且边界明确的小批量，先查已有实现和负例覆盖。
...
- `59d4190b` 让摘要与“待处理/已验证/未知”标题共用同一生成时间和计数；软件身份列出插件、Worker、Playnite、协议和运行时；URL 参数、URL 凭据和 Windows 用户路径由共享脱敏器清理。
- Worker R17-08 `2/2`、Playnite R17-08 `4/4`；合并相关回归 Worker `5/5`、Playnite `19/19`；隔离 Release solution `0 errors/2 条既有 warning`；source、XAML `24/24`、diff、WPF `0/27/162` 通过。证据：`evidence/R17-08-MAINTENANCE-REPORT-20260920.md`。
- 未触碰真实存档、媒体、云端、剪贴板或诊断目录；Demo 原目录不可用，main 用户改动、`src.zip` 和未跟踪对话框文件未碰、未合并；`.tmp/r17-08-solution` 已清理。
- 代码已提交并推送 `59d4190b` 到 `origin/codex/ui-finesse-round2`。下一可执行任务：`R18-01 连续输入基准`，先核对游戏选框搜索、取消/IME 和大库性能夹具。
...
- 当前身份 shadow 构建 `r07-07-build-98e8b244`：XAML `24/24`、solution `0/0`、Playnite `net462`；`R07FineScrollBehaviorTests 2/2`，R07 回归 `13/13`，源校验/diff check 通过。小增量 `0→36`，反向 `33→27`，末端 `154/154`，可见行 `6–9`，20 事件布局 `17`，末端 no-op 增量 `0`；加速边界外层 `32→80/432`、内层 `628`、布局 `1`。
- 夹具使用合成数据、隔离 STA WPF/offscreen logical DIP；没有物理触控板/真实 Playnite/物理 DPI/呈现帧/ETW/宿主性能证据。Demo 原始目录缺失，沿用恢复生产基线。下一可执行任务为 R07-08 resize 压力序列。

## 2026-09-18 R07-06 状态横幅预算

- 先查到生产状态实现已经区分无数据失败、保留旧数据失败/刷新和 Stale；本阶段没有新增状态模型或关闭按钮，只用真实 STA WPF 夹具验证现有横幅/错误 presenter/恢复命令和表格预算。
- `3551de81` 已提交；身份一致隔离构建 `.tmp\\r07-06-build-3551de81`：XAML `24/24`、solution `0/0`，Playnite `net462`。新增横幅行为 `4/4`，相邻空态 `2/2`、Task 响应 `7/7`、R06 详情 `2/2`、R07 滚动 `2/2`，合并 `17/17`。
- 实际行为：Task 保留旧数据时失败/刷新横幅不压零表格且重试可达；空数据失败仍显示错误 presenter；Save/Maintenance Stale 表格保留最小高度；安全模式保留明确恢复动作。更宽合跑的既有 Media 源字符串契约失败和 intentional skip 原样记录。下一可执行任务为 R07-07 触控板小增量。
...
- `test-ui-evidence-freshness.ps1` 通过文档-only、共享控件和 package identity 三类 smoke；正式扫描 `c3e67cb4` 为 `14 fresh / 0 stale`，R01-06 的 `c2399d7b` 归档身份已写入 baseline。
- R01-01～R01-07 当前证据与 freshness 事实已同步；下一批为 R01-08 skip 说明，不把本机 gated 能力或离屏审计升级为真实 Playnite/package-host 通过。Demo 原始目录、物理呈现、ETW、宿主性能仍未验。
...
- clean `toolbarprobe` 以真实 WPF 布局结果覆盖正常 settings-form、同祖先超宽 action-toolbar、同祖先不可达 action-toolbar；R00-08 以实际 Shell/WPF 路由覆盖无结果 Enter、未提交 composition、IME/方向键、候选 Enter 和焦点回返。
- 账本将 R00-07/R00-08 改为已满足。真实 OS IME、Playnite 嵌入、物理 DPI/屏幕帧、ETW、宿主性能仍未验；Demo 原始目录缺失，继续恢复生产基线。
- 下一可执行小批量为 R01-01/R01-02/R01-03 的隔离构建身份、数字可读和证据直达校正。
...
- R00-04 使用隔离 `GSC_TEST_ARTIFACT_ROOT` 保存原始基准：2,000 项、30 个不同查询、`changed_result_sets=30`、p50/p95/max=`46/47/48ms`，不可能结果约 75ms 返回 False；定向 `2/2`。
- 账本将 R00-03/R00-04 改为“已满足”。证据仍限于合成/fake、隔离 STA WPF/offscreen logical DIP；真实宿主输入、IME、Windows 偏好通知、物理呈现、ETW/宿主性能未验。Demo 原始目录缺失，继续恢复生产基线。
- 下一可执行小批量为 R00-05 上下文禁用透明度与 R00-06 媒体四行门禁。
...
- `58e1734` 新增 Playnite 编辑基线读取与编辑缓冲清理。设置页在宿主关闭事件上只对有效脏草稿调用已有原生消息框：是=CancelEdit 后放弃并关闭，否=取消关闭并恢复原字段焦点；确认异常保留窗口/草稿。临时卸载/重挂继续显示同一草稿并恢复分类/焦点。
- clean Release XAML `24/24`、构建 `0/0`、源码校验通过；R04-03 相关 `12/12`。实际 STA WPF 测试模拟窗口内容分离/重挂，确认草稿值和焦点保留，显式 CancelEdit 恢复默认值且 `HasPendingEdit=false`。RenderHarness clean `58e1734` 双主题、多尺寸和既有滚动/虚拟化/Shell/resize `render-qa OK`、357 PNG，设置 dirty/invalid 图已抽查。
- 没有自动点击真实 Playnite 原生关闭确认/按钮，真实宿主保存、取消和关闭时序、UIA、物理 DPI/跨屏、presented frame、ETW、宿主性能仍未验；使用合成设置/fake Worker/隔离目录，未写真实存档、媒体、云端。下一可执行任务为 R04-04 粘贴标准化。
...
- `8e48ac8` 增加两条实际契约测试：空选择零菜单；合成多选固定顺序六项、统一分组、Action 非空且不执行。定向 `2/2`。
- 完整 Release XAML `24/24`、构建 `0/0`、Core `83/83`、Worker `311/311`、Playnite `539/596`（57 跳过、0 失败），源码校验通过。由于菜单样式、键盘、快捷键列、翻转与关闭在 Playnite 宿主，本项完整条件外部阻塞，不能由本地字符串测试替代。
- 未启动真实 Playnite、不触发菜单 Action、不写真实存档/媒体/云端；宿主输入、屏幕阅读器、物理 DPI/IME、presented frame、ETW 和宿主性能仍未验。下一项为 R02-07 异步菜单上下文。

## 2026-09-17 R02-05 命中区与间距

- 先检查最新分支代码：图标按钮已经集中在 `GscIconOnlyButtonBase`/`GscIconOnlyDangerButton`，提供 `34×34 DIP` 命中区和 6 DIP 邻间距；工具条变体为 36 DIP，常规/紧凑文字动作使用共享高度令牌。没有把 main 的旧实现带回当前分支。
- `f03b4dd` 只增加真实 WPF 行为测试，不改生产命令：复制/删除中心命中各自按钮，间隔点不命中；窄 WrapPanel `82 DIP` 下首行是复制+删除，第三动作换行，矩形无正面积重叠。定向 `2/2`，不是字符串断言。
- 完整验证为 XAML `24/24`、构建 `0/0`、Core `83/83`、Worker `311/311`、Playnite `537/594`（57 跳过、0 失败），RenderHarness Light/Dark 56 场景通过，源码校验通过；人工查看 Media 1040×700 双主题图。`.tmp` 仅留作当前验证，文档同步后清理。
- 未启动真实 Playnite、不写真实存档/媒体/云端；真实 OS 输入、屏幕阅读器、物理 DPI/IME、presented frame、ETW 和宿主性能仍未验。下一项为 R02-06 菜单状态完整。
...
- clean-tree Release 探针绑定 `bdb99b930be12abc967df365fb82194733125a16`，即时宽度与中断宽度差异为 `0.02/0.16 DIP`，重入中间态继续向 270 DIP 展开，终态 X=0 且无活动动画；八张 PNG 和原始报告写入 `Q18-03-MOTION-REENTRY-20260915.md`，代表图已人工查看。
- Q18-03 受控视觉列升级为通过；真实 Playnite 快速输入、物理 DPI、屏幕帧和宿主时序仍不宣称完成。RenderHarness Release `0/0`，`git diff --check` 通过。
...
- 生产壳层 Light/Dark 均取得收起中间态、收起终态、快速重入终态和卸载后状态；中间态宽度约 `100/153 DIP`、Opacity `0.858/0.592`，终态宽度 `72`、Opacity `1`、X `0`、无活动动画，卸载后同样归一。为避免桌面负载吞掉中间帧，700ms 仅为审计资源覆盖，生产 token 未改。
- 人工查看浅色/深色中间图及深色终态图后，Q18-04 视觉列升级为通过；真实 Playnite Loaded/Unloaded 100 次、Rendering/ETW 和物理屏幕帧仍不由此证明，Q18-07 继续保留宿主边界。
...
- 完整 RenderHarness 首次卸载回调暴露冻结 `ScaleTransform` 异常，提交并推送 `132e6d5`（`修复按钮卸载冻结变换`）后仅对未冻结变换执行动画清理；clean-tree 完整报告 `render-qa OK`。
- 验证：解决方案/RenderHarness Release `0 warning/0 error`，`UiFinesseRound2ControlSourceTests` `19/19`，`validate-source.py`、XAML `24/24`、`git diff --check` 通过。Q06-02 仍保留真实输入视觉/宿主待验，不把源门禁写成输入截图。
...
- Q16-08/Q17-07/Q18-04/Q18-07 已将实现列升级为“代码完成”，视觉/真实宿主列仍保留待验；未运行真实 Playnite 100 次 Loaded/Unloaded、Rendering/ETW 或物理屏幕帧，不把源码门禁扩写成宿主结论。证据见 [`Q18-04-07-MOTION-CLEANUP-20260915.md`](../design/reviews/ui-finesse-round2-20260913/evidence/q13-q25/Q18-04-07-MOTION-CLEANUP-20260915.md)。
...
- 在 clean-tree `3811673` 运行 `RenderHarness audit`，人工查看 1040×700 DIP 的 Overview、Save、Media、Maintenance、Trainer、Task 六个生产工作区；重要标题、正文、状态、数字和操作均可读，审计汇总 `HIGH=0 / MEDIUM=0 / Fidelity=0`。
- 新增 `Q02-TEXT-SIZE-RENDER-20260915.md` 和六张持久化截图，Q02-07 视觉列升级为通过；Settings 路由未加载有效内容，真实宿主小窗、物理 DPI、GlyphRun 和 IME 继续保持未完成。
...
## 2026-09-15 UI 精修 Q01/Q02 当前夹具身份复核

- 提交 `8493c1a` 为 `finesseprobe` 统一写入 `Scenario/Commit/WorkingTreeClean/DpiScale/Themes/DataVolumes`，解决旧精修夹具报告缺少当前 SHA 的证据假阳性风险；目标构建 0 警告/0 错误，Dark/Light 均 `finesse-fixture OK`。
- 当前证据记录有效文本 `12` 个 0 violation、黑字负例 1、行完整 4/4 与压缩负例 3/4，并保留 `PunctuationSamples`、`NumericMetrics`、混排路径和状态样本；截图人工复核确认暗/浅主题均可读。
...
## 2026-09-15 UI 精修 Q02-04 零值、未知值与未检查语义

- 提交 `a26ef49` 增加开发夹具 `edgevalues` 模式，双主题 clean-tree 报告均记录 `SemanticEdgeValues`：`0 B`、`未知大小`、`尚未检查`、`文件 0/0 · 大小 0 B/0 B`。
- 截图人工复核确认零值、未知值和未检查状态各自可读，没有把未知格式化为零，也没有伪造时间；行完整性 4/4，压缩负例 3/4，双主题有效文本对比 0 violation。
- 随后全量 Release 门禁为 XAML `24/24`、构建 `0 警告/0 错误`、Core `83/83`、Worker `310/311`（1 skip）、Playnite `481/544`（63 skip、0 fail）。
- Q02-04 视觉列升级为通过；真实八入口业务盘点、排序键与宿主状态切换继续待验。
...
- 干净受控 WPF 运行 `120.4s/120s` 完成 `254` 循环、`886` 动作、`13` 样本、0 异常，动作 p95/max=`249.94/483.44 ms`，`>100ms=254/254`；报告为 `.tmp/q25-03-hotspot-clean-20260915/enduranceprobe-report.txt`。
- 明确边界：栈是在动作完成后的 `finally` 读取，不是 ETW 停顿采样，所以只签收“可复现动作耗时边界”，不声称已找到业务热点；真实宿主 ETW/等价工具与优化回归仍待外部条件。

## 2026-09-15 UI 精修 Q25-02 呈现回调代理专项

- 提交 `a1462ee` 补齐 `CompositionTarget.Rendering` 回调间隔的 p95、最大值和 60Hz 慢帧比例；干净 shellqa 通过，单次侧栏 `66.1/193.2/0.147`，快速二次切换 `15.3/28.5/0.038`，无动画原子终态 `36.1/36.1/0.250`。
- 代理数据不冒充屏幕呈现帧；当前 xperf DWM 会话仍因 `0x5 / Access denied` 无 ETL，Q25-02 真实呈现帧和 Q25-03 >100ms 调用栈继续待验收。证据新增 `Q25-02-RENDERING-PROXY-20260915.md`，干净报告为 `.tmp/frame-clean-20260915/shell-qa-report.txt`。

## 2026-09-15 UI 精修 Q25-04 30 分钟耐久专项

- 新增 `enduranceprobe` 并提交 `7d7e2cb`：真实 WPF STA Window 以 250 ms 节奏循环六个生产工作区、Light/Dark、Media 预览分段、列表/DataGrid 选择和可见详情入口；完整运行 `1800.4s/1800s`，`3847` 循环、`13462` 动作、`176` 样本、0 动作异常。
- 资源采样只读 `GC.GetTotalMemory(false)`、私有字节、工作集、线程和句柄，耐久代码段无强制 GC。私有字节全窗口趋势 `348,564.72 bytes/min`，预热后线程 `22–25`、句柄 `1170–1177`，中后段为有界波动；原始报告保留在 `.tmp/endurance-clean-20260915/enduranceprobe-report.txt`。
- 源码契约测试 `9/9`，RenderHarness Release 构建 `0/0`，完整探针 `enduranceprobe OK`。证据新增 `Q25-04-ENDURANCE-20260915.md`；该结果只签收受控自动验证，Playnite 宿主、真实输入/媒体/网络任务、ETW 呈现帧和 >100 ms 调用栈继续保持待验收。

## 2026-09-15 UI 精修 Q25-05 低性能回退专项

- 新增 `lowcostprobe` 并提交 `97dd0cd`：禁玻璃/禁动画下覆盖六个工作区、双主题、`1040×700`/`1600×900` 共 24/24；所有可见 Effect 为 0，Popup 透明/动画关闭，环境光/游戏背景透明度为 0，非表格水平溢出为 0。
- 实际查看深色 Overview/Task 窄窗和浅色 Maintenance 宽窗；低成本路径仍保留文字、状态、操作和数据表滚动。RenderHarness 构建 `0/0`，源码契约 `8/8`，既有 ReducedMotion/opaque fallback 测试 `2/2` 通过。
- 证据新增 `Q25-05-LOW-COST-FALLBACK-20260915.md`。真实低 Tier、宿主合成和 Q25-02～05 的其余性能边界继续保持待验收。
...
- 完成当前分支的无宿主交付回查：XAML 24/24、Release 构建 0 warning/0 error、Core 82/82、Worker 310/311（1 skip）、Playnite 468/531（63 skip），无失败；提交 `eabfc43` 已推送，远端 ref 已核对。
- Q25-08 自动验证更新为通过；最终账本审计确认 208/208 唯一任务 ID、0 缺失/异常、0 个缺失相对证据链接，`validate-source.py` 与 `git diff --check` 通过。最终结论仍为未完成，因为 clean package/install/start、物理跨屏、ETW 呈现帧、30 分钟耐久、>100ms 调用栈和低性能 Tier 仍需要真实宿主/性能工具证据。未重跑受安全策略拦截的一键安装/启停链。
...
- 测试锁定 ` · ` 分隔符、中文单位和产品名原样，并拒绝重复/两侧多余空格。Core 串行 Release 构建 0 errors、定向 `UiDisplayMappingTests` 17/17 通过；直接构建保留 3 个 NU1900 网络漏洞源告警。随后完整门禁为 XAML 24/24、构建 0 warning/0 error、Core 82/82、Worker 310/311（1 skip）、Playnite 468/531（63 skip），无失败。
...
- 修复主题资源切换测试把浅色渐变与自身比较的假阳性，改为比较 Light/Dark 的实际前景、渐变、实例和长度；新增 Popup/Tooltip/Dialog 动态资源及生产壳层广播源码门禁，提交 `46ece40`。
- WPF 源校验、Release 构建通过，Q03-07 相关定向测试 `14/14` 通过。测试只证明受控资源替换和依赖链，不证明真实打开态热切换的屏幕帧、Popup 跨屏、Playnite 主题跟随或物理 DPI。
- 证据见 `docs/design/reviews/ui-finesse-round2-20260913/evidence/q03/Q03-THEME-SWITCH-COVERAGE.md`；真实宿主测试仍因窗口枚举/安全安装边界未关闭。
...
- 打开深/浅生产资源校对 PNG 和生产 Task 浅色图，核对 Typography/ContrastGuard/GscMotion/RenderHarness 源码，发现暗色黑字与门禁假阳性、3.0 状态色用于小字、字体/四行证据含义扩大及 7 条相对链接错误。修正 BASELINE 链接并保存原始合成证据；没有改变生产 UI。
- 新增 `docs/design/UI_FINESSE_REVIEW_2026-09-13.md`、`UI_FINESSE_ROUND2_208_TASKS_2026-09-13.md` 与第二轮 208 行分维度账本，覆盖 26 类控件/排版/页面/动效/验收，每项有实施动作和专项验收。文件名沿用任务启动日 09-13，完成跨日为 09-14。
- 本轮独立隔离 `scripts/build.cmd` 退出 0：Release 0 warning/error、Core 76/76、Worker 310/311（1 skip）、Playnite 449/512（63 skip）；RenderHarness 单独构建 0 warning/error。构建不代表真实安装/宿主通过，未运行会改安装目录的一键链。
- 后续通过用户明确要求的新任务以 gpt-5.6-luna/max 持续实施；采用独立 worktree 避免主工作区 `src.zip` 阻止干净包构建，保留用户文件和打包保护。模型/创建/启动结果以工具回执为准。
- 本轮新构建 RenderHarness 的暗色 `finesseprobe` 退出 0/报告 OK，但打开新截图仍有 Numeric/部分按钮/开关/徽章黑字，已保存到第二轮 evidence 下作为当前代码重现证据。结构校验确认 208 唯一任务 ID、208 账本行、52 审阅行及本地链接有效；阶段临时构建/日志在摘录结果后清理。
...
- 文档提交后按用户要求重试阻塞任务：最终 Render QA 报告对应提交 `cacdaff` 并返回 `render-qa OK`；一键链在仅有 `?? src.zip` 时仍于打包前停止，未生成新包/安装/运行。真实宿主审计重试也在安装前被同一 clean-tree 门禁拦截，仅保留 `artifacts/ui-host-audit-finesse-final-20260913/runner-metadata.json`，无 `summary.json`，因此真实嵌入 Dashboard 仍外部阻塞。
...
- 明确不按附件从零重建设计系统：当前主题、字体、图标、按钮、玻璃和动效已有共享入口。新增队列优先处理真实短窗、错误摘要、表格/详情阅读、状态与验收工具，并将所有泛化设计建议映射到已有资源和真实业务边界。
- 本轮只更新文档，不修改生产代码、安装包或用户 Playnite；后续实施按新队列逐项验证、提交和推送。
...
- 导航、设置分类/标题（含常规与目录、设置迁移）、首页活动状态、媒体来源、维护目录操作、修改器列表、任务搜索、共享游戏图标 fallback、Disclosure chevron 和兼容 Dashboard 操作图标统一改为 `Path` 线稿；`Foreground` 由父控件/主题 DynamicResource 提供，保留选中态、状态色、命令、Binding、虚拟化和 UI Automation。
- 未替换扩展清单的 `icon.png`：压缩包中的 plugin PNG 是透明线稿导出，作为 Playnite 扩展图标存在不可见/黑底风险；当前只将 plugin-main 作为界面内主题感知 fallback 使用。
- 根据同一 ZIP 补齐 `section-general-directory` 与 `section-migration` 两个设置分类映射，未改变设置字段绑定或迁移命令。
...
- 本次只在测试文件增加运行时覆盖，生产 XAML 未变；随后从当前干净 HEAD 重跑 RenderHarness，报告为 [`.tmp/render-qa-head-clean-20260910/render-qa-report.txt`](../../.tmp/render-qa-head-clean-20260910/render-qa-report.txt)，提交元数据 `5c53e59`、`WorkingTreeClean: True`、`render-qa OK`。隔离构建目录和 Worker staging 已按规则清理，候选包未安装真实 Playnite。
...
- L32 文档链接审计扫描 123 个本地 Markdown 链接，缺失数为 `0`；同时确认当前 `.pext/.zip` 均存在。旧的一次性媒体截图链接已改为当前证据索引，不恢复已清理的临时截图。
...
- 定向缩略图回归 `1/1`，Playnite 全量 `425/488`（63 skip、0 fail）。没有放宽 `120` 断言，也没有改动生产并发上限或缓存上限。
- 提交 `559d64f` 修正 RenderHarness：Settings 状态夹具使用可访问的临时目录并在结束时清理；Sidebar 完成计时器不再被 Render 优先级的动画 tick 饿死；Media 主题探针使用实际 PageHost 内容高度，避免把外层窗口高度误传给响应式布局。RenderHarness Release 构建 `0 warning/0 error`。
- 最新干净报告为 [`.tmp/render-qa-harness-clean-20260909/render-qa-report.txt`](../../.tmp/render-qa-harness-clean-20260909/render-qa-report.txt)：Settings normal/dirty/invalid 三态正确，Sidebar rapid-toggle `secondClick=True/finalWidth=270/settled=True`，生产壳层 Media 1040/1100 的表格为 `300 DIP` 且底部区域可达。完整 render-qa 仍被 Media 嵌套预览 `126 DIP`、独立表格 `230 DIP` 和 resize `86 DIP` 门禁判失败；不将这些离屏组合直接当作真实宿主根因。
...
- 在 L42 运行基线 `398a6f0` 的隔离数据副本上先执行 L41；Playnite/CEF 受限环境出现 `拒绝访问 (0x5)`，没有窗口、扩展日志或 replay。随后用提升权限执行 L42，Release 构建无警告/错误，Core `76/76`、Worker `311/311`、Playnite `431/488`（57 skip）。
...
- 修复 Dashboard 两条分页 VM：一致性重置前从当前选中项或待恢复项捕获稳定 `TransferKey`/`BatchId`；从第一页重建后如果选中项位于后页，按 `HasMore` 继续加载并按稳定 ID 恢复，若已删除或到达末页才清理选择。没有刷新 `ItemsSource`、关闭虚拟化或把对象按名称匹配。
...
- 修复加载生命周期：控件只在已加载且可见时启动；不可见或卸载会取消、释放 pending token 并清空 `Source`；路径/宽度变化先推进 generation，异步结果需通过 generation、token 和 Dispatcher 检查后才可回写。加载器对预期文件读取/解码错误返回空值，并加入内部计数探针，不改变有界缓存、冻结或后台解码边界。
- 新增 `AsyncThumbnailLoaderTests` 的破损文件、120 项并发窗口和取消测试；新增 STA `AsyncThumbnailImageTests` 覆盖不可见不启动与旧路径不串图。RenderHarness 新增 `thumbnailprobe`：120 个 96×96 合成项初次解码 `120/120`，峰值并发 `3`，缓存 `96/96`；16 项保留窗口缓存命中 `16` 且不新增解码；破损/缺失均为空；预取消 `1` 次；100 次 12 项窗口往返请求增量 `1200`、缓存命中增量 `1089`、解码增量 `111`、活动解码 `0`；800×800→64×64 替换最终为新图（宽 `96`、合成蓝色标记 `231`）。
- 证据文件：`.tmp/l22-thumbnailprobe-final/thumbnailprobe-report.txt`。这些是合成文件与隐藏 STA Window 的生命周期/边界证据，不是 Playnite/FusionX/DPI/用户视频帧率证据；真实宿主仍需按快速切换、滚动和文件删除矩阵复测。Release 构建、全量测试、源/XAML/WPF 检查与差异检查在提交前复核。
...
- 版本详情增加来源设备、操作系统、锁定和恢复检查时间的只读摘要；空来源/系统明确显示未知。比较页增加比较范围、未变化数、带符号大小变化和三组文件标题。选中版本变化或无上一版本时清理旧比较结果，并在响应返回后校验当前游戏/左右版本 ID。
- 恢复确认展示目标版本的时间、触发方式、来源、系统、锁定与就绪摘要；确认前复制稳定游戏/版本 ID，确认后不再从可变选择读取请求目标。PreRestore、当前快照锁定、游戏关闭确认、Worker 最终阻断和撤销语义均保留。
- 验证：Release 构建 0 warning/error；Core `76/76`、Worker `303/304`（1 跳过）、Playnite `404/466`（62 跳过）；`validate-source.py`、`check-xaml.ps1` `19/19`、WPF 静态审查 `0 errors/21 warnings/172 info`、`git diff --check` 通过。完整 render-qa 失败项仍为既有 Media 小视口/预览列表与 Sidebar rapid-toggle；Save 截图和双主题多尺寸探针未见本轮新增问题。`.tmp/l19-render` 为本轮临时证据，提交前清理。

## 2026-09-09 L18 批量动作提交前摘要

- 收口媒体与任务现有批量操作的提交边界：媒体归类/忽略/恢复/预览捕获去重后的稳定 `MediaId`，归类目标和归类建议捕获 `PlayniteId`/`BatchId`；确认后不再读取变化中的 `SelectedItems`。批量归类补上目标、数量、重复/无效项和快照范围摘要，忽略/恢复沿用确认框，预览保持只读无确认。
- 任务“重试可恢复”继续按当前筛选结果和游戏/任务类型去重，缺少稳定 `TaskId` 的项纳入跳过数；确认后使用任务快照执行，并在失败详情中保留稳定任务 ID。媒体批处理和归类建议结果区分计划、成功、失败、冲突、跳过/未返回，不把部分成功显示为全量成功。
- 验证：Release 构建 `0 warning/0 error`；Core `72/72`、Worker `303/304`（1 跳过）、Playnite `404/466`（62 跳过）；`validate-source.py`、XAML `19/19`、WPF 静态审查 `0 errors/21 warnings/172 info`、`git diff --check` 通过。新增批量动作源契约测试通过。本轮未改 XAML，未重复 render-qa；真实 Playnite/FusionX 批量确认、选择变化、部分失败与录屏仍待验收。

## 2026-09-09 L17 常用筛选与工作区状态记忆

- 先盘点已有状态：任务状态/搜索与媒体筛选/搜索已保存，但任务游戏/类型在构造函数中被丢弃，任务历史范围没有设置字段；工作区导航仍按既有契约仅恢复当前会话，重启从概览开始。本阶段没有扩大为持久化瞬时对象或任务状态。
- 增加任务历史范围/时间范围设置字段和 CloneFrom 归一化；动态游戏/类型使用 pending 恢复，依据当前 Playnite 游戏库判断已删除游戏，避免最近任务分页窗口把仍存在的游戏误判为失效；非默认历史范围重启时重新激活服务端历史分页。新增媒体 `ClearMediaFiltersCommand` 与激活时显示的清除按钮，取消查询防抖后复用现有媒体分页代际链路。
- 新增/扩展旧配置默认、非法范围迁移、设置往返和媒体绑定源测试；定向持久化测试 `10/10`。Release 构建 0 warning/error；全量 Core `72/72`、Worker `303/304`（1 跳过）、Playnite `403/465`（62 跳过）；源码校验、XAML `19/19`、WPF 静态审查 0 error、差异检查通过。
- 完整 render-qa 的 Media 小视口/预览列表和 Sidebar rapid-toggle 失败与本轮无新增关联；双主题、多尺寸、resize 的 Media 工作区探针通过。临时 `.tmp/l17-render` 仅用于本轮回归，交付前清理；真实 Playnite/FusionX 重启、删除游戏和视频式操作仍待宿主验收。
...
- 保留原有 `TaskIndexedCollection`、服务端游标、筛选绑定和重试协议。新增 `TaskWaitingSummary`、`TaskRetrySummary`；队列标题区按 `TaskHasActiveFilters` 显示当前筛选摘要；批量重试 Tooltip/AutomationProperties 明确只处理当前已加载且符合筛选的结果，确认与按游戏/任务类型去重仍由原代码执行。
- 失败任务仍通过表格详情省略提示、右侧完整详情、错误码/任务 ID 和复制/安全重试入口查错，没有把 `DetailMessage` 降级成短文案。RenderHarness Fake 同步新的摘要绑定，1040×700 Task 截图仍保留 6 行首屏。
- 验证：任务定向 `36/42`（6 跳过）；全量 Release 构建 0 warning/0 error，Core `72/72`、Worker `303/304`（1 跳过）、Playnite `402/464`（62 跳过）；源码校验、XAML `19/19`、WPF 质量扫描（0 error）和差异检查通过。双主题/多尺寸/resize 的 Task 场景通过；完整 render-qa 仍受既有媒体小视口/媒体壳高度影响，真实 Playnite/FusionX、DPI、键盘和长历史操作仍待验收。

## 2026-09-09 L14 运维总览按处理顺序组织

- 维护概览原来把恢复巡检、云端关注项和隔离账本逐条排成同等权重的卡片，数据一多会把首屏变成长清单。本阶段没有改动作执行链，只给现有 `MaintenanceActionItem` 增加语义分组，并按人工处理、等待重试、例行巡检排序。
- 每个分组默认最多显示 3 条，溢出项通过显式展开器继续显示；空分组不渲染空卡片。分组计数是该组总量，列表是当前可见摘要；具体 `TransferKey`、`EntryId`、确认语义和巡检命令保持不变。Fake 夹具同步了三组形状，长标题通过截断提示和详情保留可达性。
- 新增分组边界契约，覆盖空组、单条、20 条和长标题；维护源门禁改为检查 section/preview/overflow 绑定。修正两项旧静态门禁，使其能识别 `Expander.Style` 内联 `BasedOn`，没有放宽运行时命令或模板要求。
- 验证：Release 构建 0 warning/0 error；Core `72/72`、Worker `303/304`（1 跳过）、Playnite `402/464`（62 跳过）；源码校验、XAML `19/19`、WPF 质量扫描（0 error）和差异检查通过。维护页离屏双主题、多尺寸、resize 通过；完整 render-qa 的稳定失败仍受既有媒体小视口/媒体壳高度影响，首轮另有一次侧栏快速切换未稳定，shellqa 重跑已稳定。真实 Playnite/FusionX 录屏、DPI、键盘和宿主模板仍待验收。

## 2026-09-09 L13 首页优先级与活动上下文

- 复核首页后确认 Hero 已经是单一优先级入口，但空快照会错误落到“整体状态安全”，全局活动虽然有对象名/时间却没有可达的上下文动作；云端失败计数已有 `AttentionCount`，不另造统计口径。
- 只补首页真实缺口：空库状态复用现有刷新命令；活动映射带稳定 `PlayniteId`；活动行接入一个可聚焦的 `OpenActivityCommand`，按类型进入存档、媒体、工具、云端或维护工作区并尽量恢复同一游戏。没有新增第二套 Hero、重复统计、定时器或页面级刷新。
- 新增空库/云端失败优先级测试，活动映射测试核验稳定 ID。Release 构建 0 warning/0 error；Core `72/72`、Worker `303/304`（1 跳过）、Playnite `401/463`（62 跳过）；源码/XAML/差异门禁通过。完整 render-qa 的 Overview 双主题/四尺寸/resize 通过，失败项仍为既有媒体小视口和媒体壳表格高度；真实 Playnite/FusionX 点击、DPI 与录屏待验收。
...
- `SettingsIntroDescription` 继续保留名称以兼容现有静态检查，但被统一折叠；Hero 副标题缩短。设置响应式逻辑仍保留分类栏、正文滚动、保存状态和窄窗口堆叠。
- 三态夹具发现并修正校验分支的反向布尔值：存在错误时原本摘要是红色，但保存状态却显示“已保存”。现在 normal/dirty/invalid 同一 1040×700 画布分别显示“已保存/有未保存更改/存在校验错误”，并生成离屏截图用于目视核对。
- 验证：RenderHarness Release 构建 0 warning/0 error，设置布局/三态探针通过；全量 Release 构建 0 warning/0 error；Core `72/72`、Worker `303/304`（1 跳过）、Playnite `395/457`（62 跳过）；源码/XAML/差异检查通过。完整 render-qa 仍有既有媒体小视口与侧栏动效问题，未写成 L09 通过项；真实宿主待验收。
...
- 新增 `FindingNavigationTargetResolver`。存档路由必须精确命中快照游戏；失败任务路由保留用户筛选，用临时诊断目标按 ID/名称过滤，并在加载后恢复目标任务。目标消失、名称兜底和无任务结果均给出明确状态；用户修改任务筛选会清掉临时目标。
- 连续导航取消待执行的搜索/历史防抖，只发起一次显式任务读取；现有 `taskPageGeneration`/取消保护继续负责晚到响应。没有新增写命令、全局导航框架或固定延时。
- 新增目标解析行为测试和工作区导航源契约测试。Release 构建 0 warning/0 error；Playnite 全量 `392 通过 / 62 跳过 / 0 失败`；真实 Playnite/FusionX 入口操作仍待宿主验收。
...
- 连续执行约定：验证、同步文档、独立 commit/push 后继续；已满足项凭证据跳过，外部阻塞不阻断独立项。仅规划，没有实现新功能，没有启动后台任务或扩展用户数据写入/安装授权。
- 本轮实跑 Release 构建与全量测试：0 warning/error；Core 72 通过、Worker 303 通过/1 跳过、Playnite 368 通过/62 跳过；32 个 ID 唯一顺序完整、计划与入口链接检查通过、git diff --check 通过。未改 XAML，因此未重复渲染；交付前清理 `.tmp/roadmap-0908-build` 和对应日志。
...
- 暴露隔离账本只读列表，并新增带明确确认和 `EntryId` 的再次协调 IPC；Worker 复用现有安全状态机，单条协调不会触碰其他账本，路径冲突/身份不一致仍保留文件并转人工处理。新增定向回归验证只处理选中账本项。
- 验证：Release 构建 0 警告/0 错误；Core `72/72`、Worker `303/304`（1 跳过）、Playnite `366/428`（62 跳过）；源码校验、XAML `19/19`、WPF 静态审查 `0 errors/21 warnings/172 info`、双主题/多尺寸/resize/Production Shell `render-qa OK`、`git diff --check` 通过。未运行真实 Playnite、DPI/高对比度或完整键盘人工流程。
...
- 同步收口 R04：Dashboard 独立读取全部活动任务，摘要把等待用户的云端任务计入 pending；任务总数文案区分全部历史/当前筛选，批量安全重试基于当前筛选结果去重并显示跳过原因。对应新增活动任务窗口和等待统计回归。
- 验证：Release 构建 0 警告/0 错误；Core `72/72`、Worker `302/303`（1 跳过）、Playnite `364/426`（62 跳过）；XAML `19/19`、源码校验、WPF 静态审查 `0 errors/20 warnings/172 info`、`render-qa OK`、`git diff --check` 通过。未运行真实 Playnite。
...
- 本阶段未改生产代码；`python scripts/validate-source.py`、`git diff --check`、Release 构建和全量测试通过。构建保留 1 个既有 `NU1900` 网络审计警告；全量 Core `72/72`、Worker `300/301`（1 跳过）、Playnite `363/425`（62 跳过）。

## 2026-09-07 动效门控与即时终态

- 复核发现动画设置或 Windows 动画偏好在侧栏过渡进行中关闭时，原实现只影响下一次点击，当前宽度/透明度/位移时钟仍可能继续运行。新增 `NormalizeMotionIfDisabled()`，集中取消侧栏动画并应用当前收起/展开终态；下一次点击继续沿用同一安全路径。
- Dashboard 在视觉设置刷新时同步清理生产 Shell 和自身已知页面过渡，避免关闭动画后入口、筛选、详情、状态胶囊、对话框或任务详情停留在中间态；不改变真实绑定、命令和业务状态。
...
- `MediaCenterView` 新增锚点代际和统一失效路径：切换游戏、收件箱模式、ViewModel、页面生命周期或集合上下文时，使旧回调失效并清理锚点、选择与计时器；收件箱恢复额外检查模式。ViewModel 的 `PropertyChanged` 订阅随 Attach/Detach 成对维护。
- 恢复重试链路不再在单次选择恢复的 `finally` 中提前清除占用标记；成功、无法恢复提示和上下文失效分别负责释放，避免旧 `SelectionChanged` 覆盖待恢复 ID。
- 新增真实 STA WPF `Window` 行为回归，验证旧回调在上下文失效后不显示过期提示，以及锚点被窗口裁掉时提示可见且恢复锁释放。媒体锚点定向测试 `5/5` 通过。
- 验证：Release 构建 0 错误（1 个 `NU1900` 网络审计警告）、Core `72/72`、Worker `300/301`（1 跳过）、Playnite `362/424`（62 跳过）、源码校验、XAML `19/19`、`git diff --check` 通过。无 XAML/布局改动，未重复生成渲染产物；真实 Playnite、DPI/高对比度、完整键盘和大库滚动仍需人工验收。
...
- 实现 `query_revisions` 持久化修订表及幂等触发器，分别覆盖云端队列/重试/游戏和媒体影响字段、归类批次/条目。两个 Worker 响应增加不透明 token 和 reset 原因，查询前后发现变化都会阻断本次页结果。
- ViewModel 翻页携带 token；reset 时清空旧窗口、清除过期选择、设置 `StatusMessage` 并最多自动从第一页重载一次，保留用户可见的刷新入口。没有全量拉取大列表。
- 验证：Release 构建 0 错误；Core `72/72`、Worker `300/301`（1 跳过）、Playnite `358/420`（62 跳过）；XAML 19/19、源码门禁、WPF 0 errors、`git diff --check` 通过。NuGet `NU1900` 为访问 nuget.org 漏洞服务失败的环境警告，未影响编译/测试；未运行真实 Playnite。
...
- 新增 `MediaCloudRetryResultDto`，明确区分已提交、按游戏策略暂停、无法提交；安全模式、全局云端关闭、Rclone 未配置、任务失败/取消均由 UI 展示真实原因，不再无条件显示“已提交”。Worker 回归新增策略暂停和云端关闭两项。
- 新增 `MediaTabIndex`/`SaveTabIndex` 双向绑定。首页待归类动作路由到媒体“待归类”，诊断存档路径入口路由到“路径与校验”；普通侧栏切换仍保留页面上次标签。Playnite 定向契约测试 4/4，Worker 媒体测试 10/10，Release 构建 0 warning/error。
- 未验证真实 Playnite 宿主、真实 Rclone/远端和用户媒体数据；下一阶段回到质量报告 Q4-03，先处理紧凑维护页列表与详情空间。
...
- 实际运行 RenderHarness 并审看七类页面代表截图；优先媒体紧凑按钮溢出/主表裁剪、云队列明细入口、可撤销批次历史、任务密度。区别夹具缺字段与生产缺陷，补充当前静态文案/状态显示改进。
- 保留六张未修改原始截图与渲染报告作为文档证据，其他本轮临时构建/截图/日志清理；没有修改 XAML、业务代码、版本或安装状态。
- Release 0 warning/0 error；Core 65/65、Worker 294 通过/1 跳过、Playnite 341 通过/62 跳过、XAML 19/19；RenderHarness `render-qa OK`，源码校验与差异检查通过。没有本轮真实 Playnite/远端操作或新增故障注入证据。
...
- 校验成功/失败使用操作 ID CAS 收尾，迟到的旧校验不能覆盖新的上传代际；失败仅记录 `RCLONE_CHECK_FAILED`/认证失败，取消和异常恢复原保证，基础游戏云端投影为尽力写入。
- 启动恢复只重新排队 Upload 型未完成条目，Verify 型条目恢复原快照或明确 `CheckFailed`，不因校验中断发起上传；未增加任何本地或远端删除路径。
- 新增取消、异常、check 失败、代际覆盖、重启恢复和旧库迁移回归。验证：隔离 Release 构建 0 warning/0 error，Core `65/65`，Worker `293/294`（1 跳过），Playnite `339/401`（62 跳过），XAML `19/19`，源码校验、XAML 检查和差异检查通过。
- 未执行真实 Playnite、真实云端凭据/断网、硬杀端到端和跨进程并发。

## 2026-09-06 V2-04 IPC 请求身份与重放指纹

- 扩展 `ipc_request_ledger`，保存协议版本和规范化 JSON 负载 SHA-256 指纹；同 RequestId 仅在 type、协议和负载一致时重放，属性顺序变化保持兼容。
- 拒绝同 ID 的不同类型、协议或负载，返回稳定的 `REQUEST_ID_CONFLICT`；受保护写请求缺少 ID 返回 `REQUEST_ID_REQUIRED`，两种情况都不执行写操作。旧账本无指纹时不重放旧结果。
- 完成项按 7 天、每轮最多 256 行清理，中断证据保留 30 天；Named Pipe 服务增加每小时低频维护任务，维护路径只清理终态，不把当前 Worker 的 in-flight 标记为中断。
- 新增账本冲突、属性顺序、旧记录、迁移和保留策略回归。验证：IPC/迁移定向 `20/20`；隔离 Release 构建 0 warning/0 error，Core `65/65`，Worker `288/289`（1 跳过），Playnite `339/401`（62 跳过），XAML `19/19`，源码校验与差异检查通过。
- 未执行真实 Playnite、跨版本旧 Worker/插件组合和硬杀端到端重放。

## 2026-09-06 V2-03 健康巡检候选公平与恢复游标

- 将候选选择改为稳定的 `PlayniteId`、`CreatedUtc`、`BackupId` 排序和完整复合游标轮转；持久化 in-flight 候选在重启后优先恢复，避免新增优先级更高版本导致当前候选被静默跳过。
- 选定候选后先落盘身份再检查会话、申请游戏锁或读取归档。候选状态写入失败会停止本轮，不创建归档失败 readiness/finding，并保留可重试状态故障语义。
- 为被运行中游戏/同游戏操作推迟的候选新增 SQLite 下次尝试账本，本轮可继续检查其他游戏；全部候选新鲜时返回 `UpToDate`，不重复读取归档。
- 新增 5 项行为回归，健康巡检定向测试 `11/11`；隔离 Release 构建 0 warning/0 error，Core `65/65`，Worker `285/286`（1 跳过），Playnite `339/401`（62 跳过），XAML `19/19`，源码校验与差异检查通过。
- 未执行真实 Playnite、硬杀/断电、跨进程并发或长时调度压力测试。

## 2026-09-06 V2-02 健康巡检调度与计划写入

- 修复健康巡检后台在手动巡检占用执行锁时的忙等：争锁失败后按 250ms 有界退避；后台循环外围捕获临时存储/调度异常，按 1s 退避后继续，不因单轮异常退出。
- 将巡检计划更新与执行结果更新拆为两个 SQLite 契约。计划写入只覆盖启用、间隔、过期、预算和下次时间；执行写入只覆盖运行状态、游标、结果计数和完成时间；完成时重新读取最新计划计算下次时间。
- 新增可控闸门测试，证明手动巡检持锁期间后台 700ms 内争锁次数受限；新增并发状态测试，证明旧执行 DTO 不会覆盖新计划字段。
- 验证：定向健康巡检测试 `6/6`；隔离 Release 构建 0 warning/0 error，Core `65/65`，Worker `280/281`（1 跳过），Playnite `339/401`（62 跳过），XAML `19/19`，源码校验与差异检查通过。未取得真实宿主长时调度证据。

## 2026-09-06 V2-01 媒体归类提交与恢复协调

- 将媒体归类应用/撤销的文件操作改为带 SQLite 操作账本的两阶段流程：先记录 `Planned`，移动后记录 `Moved`，媒体行、批次条目和账本在同一事务中提交为 `Committed`；批次提交失败不会再留下“媒体已改但文件已回搬”的不一致窗口。
- Worker 启动在常规任务恢复前协调未完成操作。可确认已提交的记录补齐为 `Committed`；仍是原业务状态的记录按源/目标哈希安全恢复并标记 `Aborted`；无法唯一对应时保留 `RecoveryRequired`，不删除或覆盖未知内容。
- 审计写入改为提交后的尽力操作，失败在结果消息和日志中提示但不回滚已提交业务；取消/补偿使用独立令牌，并正确处理从原始捕获文件复制归档的回滚边界。
- 新增三个故障/行为回归：批次条目 SQLite 更新失败后的启动恢复、审计 INSERT 失败不回滚、移动后取消仍能恢复；既有正常应用、撤销、冲突测试继续通过。
- 验证：`scripts/validate-source.py`、`scripts/check-xaml.ps1`、`git diff --check` 通过；隔离 Release 构建 0 warning/0 error，Core `65/65`，Worker `278/279`（1 跳过），Playnite `339/401`（62 跳过），XAML `19/19`。未执行真实 Playnite、用户媒体目录或硬杀时序。

## 2026-09-06 已完成功能复查与文档更新

- 用户要求“检查已完成内容，只更新文档”。核对 `8018cee`、当前事实和 R/U/F/E 实现，新增 [第二轮复查任务包](FOLLOWUP_REVIEW_2026-09-06.md)，记录 7 项后续修复/优化及 3 组扩展，不重复列已解决的锁释放、清理预览和首屏分页问题。
- 核心发现是媒体提交后补偿不一致、巡检竞争循环与候选落盘、云端校验失败终态；详细证据、实现入口和故障验收均在任务包。只审阅源码，未新增这些路径的行为复现或修改业务。
- Release 0 warning/0 error；Core 65/65、Worker 275 通过/1 跳过、Playnite 339 通过/62 跳过；XAML 19/19，源码验证和差异检查通过。本轮未验证 Worker 硬重启/5 项管道行为（跳过），未运行 RenderHarness/full-stress/真实 Playnite。隔离构建和临时日志清理后，以任务包记录保存结果。
...
- full 夹具为 2,000 游戏、20,000 备份、10,000 任务、5,000 媒体、500 工具；seed 用时 `105999 ms`，20 轮 Worker 读/事件/原子写/操作锁模拟用时 `427 ms`。游戏查询首查/热查为 `17/14 ms`，任务页首查/热查/下一页为 `92/3/3 ms`，任务搜索 `4 ms`，媒体页首查/热查/下一页为 `3/1/0 ms`，媒体搜索 `0 ms`；查询分配 `6635504` bytes，模拟分配 `61057744` bytes。
- stress 夹具为 10,000 游戏、20,000 备份、10,000 任务、50,000 媒体、500 工具；seed 用时 `242832 ms`，模拟用时 `1529 ms`。游戏查询首查/热查为 `83/84 ms`，任务页首查/热查/下一页为 `93/3/2 ms`，任务搜索 `5 ms`，媒体页首查/热查/下一页为 `4/0/1 ms`，媒体搜索 `15 ms`；查询分配 `30162656` bytes，模拟分配 `294692392` bytes。
- 两档均通过 `managedGrowth <= 256 MiB`、句柄/线程增长上限、首屏/搜索/分页行数、订阅者残留和原子写临时文件残留断言，报告错误列表为空。该阶段证明的是隔离 Worker/SQLite 数据规模和资源边界，不等同真实 Playnite 冷/热首屏、UI 分配、帧间隔或用户目录性能；这些仍需宿主/目标机验收。
...
- 应用与撤销只移动可恢复的归档副本，原始媒体文件和真实存档不会删除或改写；撤销允许在预览过期后执行，但要求批次仍为已应用且条目保持应用后的 `Pending` 状态，Worker 重启后可从 SQLite 继续撤销。移动或数据库更新失败会回滚已移动副本，并保留部分失败结果。
- MediaCenter Inspector 增加“生成归类建议 / 应用高置信建议 / 撤销上次建议批次”和虚拟化预览列表；既有手工归类、忽略恢复、选择器和分页列表保持不变。未引入图像删除、OCR 或云端 AI 上传。
- 新增来源规则/重叠会话/未知时间、应用撤销重启、手工覆盖冲突和归档安全回归。验证：Release 0 warning/0 error；Core `65/65`、Worker `275/275`、Playnite `338/400`（62 跳过）、XAML `19/19`；`validate-source.py`、WPF 校验、`git diff --check` 和 RenderHarness 多尺寸/双主题/resize/侧栏探针均通过并输出 `render-qa OK`。真实 Playnite、真实用户媒体目录、长时批量和多进程并发仍需人工复核。
...
- RenderHarness 增加 2000×1100 宿主的 Measure/Arrange、LayoutUpdated、Rendering 帧间隔和快速往返探针，同时对比原子切列宽路径。独立 shell probe：单次过渡 `4` 次 Arrange、约 `9.9ms`；快速往返最终宽度 `270` DIP 且完成；无动画原子切换约 `4.0ms` Arrange。成本可见但不支持移除现有过渡。
- 新增快速目标/取消清理源码回归；完整 Release 构建 0 warning/0 error，Core `65/65`、Worker `260/260`、Playnite `338/400`（62 跳过），XAML `19/19`，完整 RenderHarness 多尺寸/双主题/resize 与侧栏探针均为 `render-qa OK`。测量为离屏夹具，不等同真实 Playnite 帧率；2000 游戏/5000 媒体真机压力、DPI 和高对比度仍待人工复核。

## 2026-09-05 U01 任务页视口与状态试点

- TaskCenter 增加独立的页面状态模型：加载中、真正无数据、筛选无结果、无旧数据失败和保留旧数据失败；失败或刷新期间已有任务继续留在列表，并显示最近更新时间、错误摘要和“重试”入口，筛选无结果提供“清除筛选”。
- 任务页在低高度窗口把次级摘要从 `84` DIP 收紧到 `64` DIP，主表仍保留 `236` DIP 最小高度；宽屏主列表与 Inspector、窄屏紧凑详情入口、列表内部滚动、Recycling 虚拟化和既有命令/绑定均未改变。
- 新增任务页状态与响应式回归测试。验证：Release 构建 0 warning/0 error；Core `65/65`、Worker `260/260`、Playnite `337/399`（62 跳过），XAML `19/19`、`validate-source.py`、`git diff --check` 和 RenderHarness 四组尺寸/主题/缩放转换均通过并输出 `render-qa OK`。真实 Playnite 宿主、高对比度、100/125/150/200% DPI、长错误详情和大数据量压力仍待人工复核。
...
- Worker 启动时协调未完成条目：只有已持久化 `IndexRemoved` 且大小仍匹配的已知文件允许自动删除；早期状态优先恢复原路径；原路径已有文件、路径越界、大小不符或未知 `.pending` 文件均不覆盖、不删除，并保留人工恢复状态。
- 预览和维护健康报告显示待处理条目、实际隔离占用和待人工恢复数量；结果区分移入隔离字节、真正释放字节和当前隔离区占用，物理删除失败不再虚报释放空间。
- 新增 SQLite 账本、未知隔离文件保护、启动恢复/恢复原路径和维护报告回归测试。验证：Worker Debug/Release `247/247`、Core `65/65`、Playnite `331/388`（57 跳过），Release 构建 0 warning/0 error，`validate-source.py` 和 `git diff --check` 通过；真实 Worker 崩溃时序仍待人工或隔离宿主验证。

## 2026-09-05 R02 全局保留清理候选身份与操作互斥

- 全局保留预览新增 Worker-owned `PreviewId` 和服务端完整指纹快照；Apply 原子消费预览，拒绝重复/过期/重启后旧预览，不再信任客户端候选数量或预计体积。
- 执行前后核对候选 ID、规范化路径、索引与文件元数据、保护状态和策略指纹；逐游戏取得共享 `GameOperationLock`，备份元数据锁定、策略修改、恢复可恢复性写入和仓库索引重建也使用同一互斥通道。
- 增加 reparse point/junction 边界检查和移动前文件身份复核；候选变化或游戏忙碌时跳过并在结果摘要中明确计数，替代候选不会被误删。
- 新增候选替换、同路径同大小替换、策略变化、重复提交和锁占用测试。验证：Worker `244/244`；Release 0 warning/0 error；`validate-source.py` 和 `git diff --check` 通过。真实 Playnite/Rclone 仍待人工验证。

## 2026-09-05 R01 任务终态写入失败释放锁

- 将 `TaskCoordinator` 的状态存储依赖抽为最小 `ITaskStatusStore` 接口，生产 SQLite 实现保持不变，测试可以只注入终态写入故障，不需要重构整个持久化层。
- 终态写入与令牌移除/游戏锁释放分离；持久化异常记录任务 ID、游戏 ID、任务类型、原始业务终态和异常后继续清理。事件仍在成功落盘之后才发布，未落盘的成功不进入 UI 事件流。
- 新增成功、业务失败、取消三条终态路径的故障注入测试，验证后续同游戏任务、其他游戏任务、Cancel 清理和终态事件边界。
- 验证：Worker Debug 构建 0 warning/0 error；Worker `238/238` 通过；`git diff --check` 通过。真实 Worker/Playnite 生命周期故障注入仍待人工或隔离宿主环境。

## 2026-09-05 项目审阅与后续实施方案

- 按用户要求审阅近期提交、项目记忆、Worker 任务/清理/媒体/云端/IPC、WPF 筛选和任务统计、共享资源及现有测试；交付 [15 项完善方案](IMPROVEMENT_ROADMAP_2026-09-05.md)，包含优先级、代码入口、实施步骤、验收标准、并行边界与可直接交给其他 AI 的提示。
- 明确区分源码异常路径、需行为复现风险和产品增强；重点包括终态写入失败后的锁释放、清理候选身份与互斥、隔离账本、任务统计口径、媒体按需分页。此次只写方案，未修复业务或修改 UI。
- 执行 Release 隔离构建和现有全量测试：0 warning/0 error；Core 65/65、Worker 235/235、Playnite 331 通过/57 跳过；XAML 19/19、validate-source.py、git diff --check 通过。
- 未执行本轮真实 Playnite、RenderHarness、实际云端或故障注入；缺失的 AcrylicFork Design 路径与文档漂移已列为治理项。临时隔离构建目录和日志在交付前清理，结果保留在方案文档。
...
- 新增 `UiFilterSelection.Synchronize`，在生产 Shell 和兼容 Dashboard 的加载/平台选项恢复路径把三个选择框对齐到共享 `GamePickerViewModel`，修复合法但过期的“全部”显示值继续使用“已安装”过滤的竞态；新增 STA 回归测试。
- 验证：Core `65/65`、Worker `234/234`、Playnite `329/386`（57 跳过）；Release 编译 0 警告/0 错误；源码校验、XAML 19/19、WPF 静态审计和双主题多尺寸 Render QA 通过；发布包 0.6.70 已安装并核对 DLL 哈希。真实宿主自动化未找到侧栏入口，第二台机器的目标游戏筛选仍需人工复核。
...
- 批量候选从现有 `Tasks` 计算，按游戏与任务类型去重并只取最新记录；`BackupAll`/`MediaInbox` 各只执行一次。执行前二次确认，复用单任务重试分流，单项失败继续处理并在最后汇总。
- 验证：Release 构建 0 warning/0 error；Core `65/65`、Worker `234/234`、Playnite `326/383`（57 跳过）；源码校验、WPF 静态审计、`git diff --check` 通过。真实 Playnite 任务中心和长任务关闭行为仍需人工验证。

## 2026-09-03 Playnite 游戏菜单补充媒体同步

- 从产品入口完整性检查发现，Playnite 游戏右键菜单已有“立即备份”、历史、恢复点和游戏工具，但没有直接同步媒体入口；用户不必先打开 Dashboard 才能归档媒体，因此补充该快捷操作。
- `GameSaveCenterPlugin` 新增多选媒体同步菜单项：媒体关闭时提示并返回；启用时沿用 Worker/设置同步、游戏描述写入、`MessageTypes.SyncMedia` 和云端上传开关，不新增业务协议。
- 验证：Playnite `325/382`（57 跳过）、源码校验通过；本轮完整 Release 构建与 Core/Worker 回归在最终交付前收口，真实 Playnite 菜单命中仍需手工验证。

## 2026-09-03 游戏级云端状态汇总媒体上传

- 继续从产品完整性检查云端提示时发现，游戏卡片/选择器读取的 `games.cloud_state` 主要由存档上传维护；已归类媒体即使失败或等待上传，游戏仍可能显示“已上传”或“未启用”。
- 在现有 Dashboard 媒体聚合 SQL 中加入媒体云端状态计数，按失败、等待重试、待上传、已上传优先级合并到游戏级状态；只统计 `Assigned` 媒体，未增加 N+1 查询，也不改变媒体任务和 Rclone 语义。
- 验证：新增 Worker 持久化回归通过；Worker `234/234`、Core `65/65`、Playnite `321/378`（57 跳过），Release 构建 0 warning/0 error，源码校验和 WPF 静态审计通过。真实 Rclone 断网恢复、媒体长传输和宿主显示仍需人工观察。
...
- 下载接口改为接收可等待的 `Func<long, long?, Task>`；下载循环按原有取消令牌等待进度回调，GameToolService 以 5–80 的百分比只报告每个新百分比，保持进度顺序并把失败交给当前任务协调器处理。下载/解压/哈希/安全上限未改变。
- 验证：Release 构建 0 warning/0 error；Core `65/65`、Worker `227/227`、Playnite `319/376`（57 跳过）；源码校验通过。真实 FLiNG 网络下载、慢速磁盘和取消时机仍需人工观察。
...
- 现在退出时先取消 `lifetimeCancellation`、停止任务通知计时器，再停止本插件持有的 Worker。`FireAndForget`、Worker 启动、库回调、任务通知轮询、游戏启动/停止会话写入以及目录同步均在退出边界提前返回；同步闸门等待也绑定生命周期令牌，保护 Worker、Dispatcher 和通知状态不被关闭中的旧回调触碰。
- 验证：Release 构建 0 warning/0 error；Core `65/65`、Worker `226/226`、Playnite `319/376`（57 跳过）；源码校验通过。未改 XAML，既有 WPF 静态审计和 Render QA 结果不变；真实 Playnite 退出、Worker 重启和 Dispatcher 关闭仍需人工观察。

## 2026-09-01 异步操作返回上下文保护

- 复查发现，保存媒体元数据、媒体批量修改和存档元数据时，用户若在 Worker 返回前切换游戏或条目，旧响应可能更新当前列表并清理新条目的 dirty 状态；媒体摘要也可能被旧游戏结果覆盖。现在按原游戏/条目/请求时编辑值校验，只在原上下文仍可见时更新集合、摘要和对应 dirty 标记。
- 同一类问题还存在于媒体重新归类、备份完成提示、校验/恢复准备、策略模板应用、备份比较/保留预览和进程映射输入清理；这些流程现在都捕获稳定参数，返回提示和回写不再读取后来选中的对象。
- 验证：Release 构建 0 warning/0 error；Core `65/65`、Worker `226/226`、Playnite `318/375`（57 跳过）；源码校验通过。未改 XAML，既有 WPF 静态审计与双主题多尺寸/Resize/Shell Render QA 结果继续有效；真实宿主快速切换仍需人工观察。

## 2026-09-01 详情编辑草稿与游戏摘要刷新保护

- 复查发现，存档详情的备注/锁定和媒体详情的备注/收藏是绑定到 ViewModel 的编辑草稿；详情刷新即使仍是同一 `BackupId`/`MediaId`，原选择器 setter 也会重新从 DTO 写入这些字段，可能覆盖用户尚未点击保存的输入。
- 为四个编辑字段增加独立 dirty 标记。刷新同一条目时只回写未修改字段；切换条目、清空选择时完整重置；单项/批量保存成功后清除对应 dirty 标记，保证服务端返回值可以重新同步。
- 复查还发现，游戏策略直接编辑 `SelectedGame.Policy` 时，游戏库快照替换会覆盖未保存策略；现在以当前游戏基线识别本地草稿，并只在界面展示集合中保留该草稿。游戏选择器在同一条目数据刷新后补发 `SelectedGame` 通知，生产壳层当前游戏标题和平台摘要可以同步更新。
- 策略保存请求现在在发往 Worker 前捕获游戏 ID、名称和策略副本；返回期间切换游戏时只更新原游戏的基线，成功提示也不会串到新选中的游戏。
- 验证：Release 构建、Core/Worker/Playnite 全量测试、源码/WPF 门禁、双主题多尺寸/连续 Resize/Shell Render QA 均通过；真实宿主输入期间自动刷新仍需人工观察。

## 2026-09-01 媒体收件箱与详情刷新一致性

- 复查发现，媒体收件箱切换到“已忽略”时如果已有其他操作占用 `IsBusy`，原实现会直接丢弃加载请求；请求返回时也会用旧快照覆盖用户刚选择的媒体和目标游戏。现在以模式代际号和当前模式校验响应，忙碌期间只保留最后一次模式请求，`RunAsync` 完成后自动补加载。
- 媒体刷新仍会更新未归类/已忽略缓存，但只有当前请求有效时才切换可见集合；目标游戏和收件箱媒体按当前选择 ID 保留。存档版本和当前游戏媒体详情刷新也按 `BackupId`/`MediaId` 保留选择，条目不存在时才回退第一项。
- 媒体收件箱新增共享 Worker 离线状态面板，Worker 不可用时不再把空缓存误报为“没有媒体”；健康状态下的页面布局、命令、Binding、DataGrid 虚拟化和滚动未改。
- 验证：Release 构建 `0 warning/0 error`；Core `65/65`、Worker `226/226`、Playnite `315/372`（57 跳过）；`validate-source.py` 通过；WPF 静态审计 `0 error/18 warning/172 info`；双主题、多尺寸、连续 Resize 与生产 Shell `render-qa OK`。真实 Playnite 快速切换、Worker 重启和 DPI 仍是人工边界。

## 2026-09-01 修改器目录加载状态反馈

- UI 复查发现，FLiNG 搜索结果和可下载版本在请求尚未返回时，空集合会立即触发“没有匹配/请选择版本”，容易让用户把正常等待误判为无结果。
- 为目录搜索、目录同步和版本读取增加独立的加载状态属性；请求期间显示共享 `WorkspaceStatePresenter` 加载态，只有请求完成且集合仍为空时才显示空状态。现有命令、Binding、自动读取版本和安全下载流程保持不变。
- 验证：Release 构建 `0 warning/0 error`；Core `65/65`、Worker `226/226`、Playnite `313/370`（57 跳过）；`validate-source.py` 通过；WPF 静态审计 `0 error/18 warning/172 info`；双主题、多尺寸、连续 Resize 与生产 Shell 渲染审计 `render-qa OK`。用户已确认当前版本可正常编译并在 Playnite 运行；本次新增改动仍不把离屏审计当作全量真机矩阵替代。

## 2026-09-01 修改器版本读取竞态与逐项操作优化

- 复查发现，FLiNG 列表每行的“读取版本”按钮没有传递当前行对象，点击非选中行时实际读取的是旧的全局选中项；快速切换条目时，较慢的旧请求也可能覆盖新条目的版本列表。
- 按钮现在传递并选中被点击的目录项；版本请求保存目录 ID与代际号，只接受仍对应当前选择的响应。忙碌期间的新选择会排队，并在当前操作结束后自动读取最新条目，避免列表、详情和下载按钮错位。
- 验证：Release 构建 `0 warning/0 error`；Core `65/65`、Worker `226/226`、Playnite `314/371`（57 跳过）；`validate-source.py` 通过；WPF 静态审计 `0 error/18 warning/172 info`；双主题、多尺寸、连续 Resize 与生产 Shell 渲染审计 `render-qa OK`。新逻辑未改变命令安全边界、下载校验或数据契约。

## 2026-09-01 IPC 媒体边界与任务通知缓存优化

- 复查稳定性和性能路径时发现，普通当前游戏媒体列表虽然由 SQLite 允许最多 5000 条，但 IPC 分发层没有单独限制，未来异常请求可能制造接近 4 MiB 命名管道上限的大响应；新增 Worker 端 1–1000 条页大小夹限，保持当前 UI 的 1000 条需求不变。
- 任务通知去重 ID 原先永久保存在插件生命周期内的 `ConcurrentDictionary`，长时间运行会随已结束任务数量增长；改为容量 4096 的线程安全 FIFO 集合，保留大小写不敏感去重并淘汰最旧 ID，不影响任务状态或 SQLite 数据。
- 回归：Release 构建 `0 warning/0 error`；Core `65/65`、Worker `226/226`、Playnite `312/369`（57 跳过），`validate-source.py` 通过，WPF 静态审计 `0 error/18 warning/172 info`。真实 Playnite 宿主、DPI、任务历史重放和真实命名管道大响应仍属手工验证边界。

## 2026-09-01 FLiNG 目录解析收口

- 检查剩余未提交的 FLiNG 修改，确认原实现只完成了解析方法抽取和基础 canary；补齐通用 `<a>` 属性顺序/空白匹配、相对与协议相对链接解析、HTML 解码、FLiNG HTTPS/路径边界和规范 URL 去重。
- 目录页剥离追踪查询参数，详情页下载链接保留必要查询参数但移除 fragment；外站与错误路径不会产生目录项或下载版本。生产详情页按真实页面 URL 作为相对链接基准。
- 验证：Worker `222/222`、Core `65/65`、Playnite `310/367`（57 跳过）、`validate-source.py` 和 `git diff --check` 通过。真实站点/下载和 Playnite 宿主仍是手工边界。

## 2026-09-01 媒体云端重试收口

- 发现并修复媒体云端复制失败后的重试遗漏：原逻辑只在本轮新增媒体数大于 0 时上传，导致本地已有 `Failed`/`Pending` 媒体再次同步时直接跳过；公共 Inbox 在没有新归类文件时也可能没有待上传游戏。
- 新增 SQLite 待上传游戏查询和云端状态索引；单游戏与公共 Inbox 均在复制前把媒体状态写为 `Pending`，成功写为 `Synced`，支持现有 MediaSync/MediaInbox 任务重试恢复。
- 媒体/游戏云端状态显示补齐中文文案；Worker 测试 `220/220` 通过。真实 Rclone 和 Playnite 宿主云端回归仍待用户执行。

## 2026-08-31 UI/可靠性审计收口

- 完成整库备份耐久化：新增 `backup_all_jobs`，主任务提交后立即返回，逐游戏子任务继续使用 `TaskCoordinator` 的真实状态/事件；Worker 初始化恢复未完成任务，Dashboard 显示整库任务类型并支持重试。为避免请求连接关闭误取消后台任务，后台执行优先绑定 `ApplicationStopping`。
- 修复大库匹配的遗漏路径：优先级预算只用于排序，所有待匹配条目都进入后台队列；未匹配状态 6 小时后可重试，Worker 停止时取消后台循环。媒体同步增加采样签名与有限期缓存清理，保留完整 SHA-256 作为权威去重。
- 修复生产 Shell 窄标题区：`HeaderRow` 自动高度、操作区 `WrapPanel` 和实际内容宽度约束；720 DIP 截图曾复现右侧按钮裁切，改后按钮完整换行。剪贴板复制的 COM 重试改为异步延迟，避免 `Thread.Sleep` 卡住 UI 线程。
- 新增 `RenderHarness shellqa`，覆盖 720/960/980/1040 DIP 的标题区越界检查与截图；结果 `shell-qa OK`。本轮自动化：Core `59/59`、Worker `219/219`、Playnite `310/367`（57 跳过），构建 0 警告/0 错误，WPF 静态审计 0 error/18 warning/172 info。
- 未执行真实 Playnite/隔离数据根/DPI/高对比度/Worker 重启人工矩阵；这些仍是交付后的手工验收边界。
...
- 当前滑块范围是 20–100，但实际 Blur 半径只有约 16–34 DIP，导致拉到 100 时仍像较低强度；默认值的视觉模糊也不够明显。

**实现内容：**

- `AdaptiveThemePalette` 新增统一的 `BlurRadiusForStrength` 映射：20%→20 DIP、默认 78%→78 DIP、100%→100 DIP，主界面游戏背景和设置页环境 Blur 同步使用该比例。
- Blur 仍只作用于 Shell 的静态游戏图片层和设置页环境层；卡片、文字、表格、列表和滚动区域不增加 BlurEffect，避免文字变糊、虚拟化成本上升和输入可读性下降。
- 保持 `EnableGlassEffects=false`、高对比度、无背景图时返回真实 `null` 的降级路径；没有修改图片居中裁剪、游戏背景开关、主题回退或现有材质层级。
- WPF 资源回归测试现在锁定 78% 为 78 DIP、100% 为 100 DIP，并验证两者差值确实随滑块比例增加。

**验证结果：**

- WPF 资源定向测试 119 通过、39 跳过；全量 Playnite 测试 297 通过、57 跳过、0 失败；Release 构建 0 warning、0 error。
- `.tmp/ui-qa-glass-strength-v1/render-qa-report.txt` 为 `render-qa OK`，浅色/深色、多窗口尺寸、设置页、列表滚动和 resize transition 均通过。
- 真实 Playnite 宿主的 100% 强度帧率、DPI 和个人观感仍需用户安装后复核；当前 RenderHarness 只证明资源、布局和主题回退无回归。

## 2026-08-25 UI-329 刷新链路与背景取色性能收口

**巡检结论：**

- Dashboard 的自动快照刷新虽然不改变当前游戏，但此前仍会重复读取图标并启动同一张背景图的异步解码；大库或 Worker 短暂忙碌时会把不必要的工作叠加到 UI 线程附近。
- 背景材质只需要五个采样点，却先把整张解码后的 32 位位图复制到多 MB 临时数组；这会增加频繁切换游戏时的瞬时分配和 GC 压力。
- 生产壳窗口拖拽会产生连续 `SizeChanged`，每次都立即重算页头和所有页面响应式布局；游戏筛选选项异步重建也会排队重复的默认值恢复。

**实现内容：**

- Dashboard 只有在选中游戏 ID 真正变化时才刷新图标和背景；页面重新显示时增加一次性 `EnsureSelectedGameBackgroundLoaded`，仅恢复被卸载取消的当前背景加载，不把解码重新塞回每次轮询。
- 背景渐变改为从已转换的 `BitmapSource` 逐点读取五个 BGRA 像素，移除整帧 `byte[]` 复制；材质颜色、采样位置和透明度语义保持不变。
- 生产壳的窗口尺寸变化合并到 `DispatcherPriority.Render`，一帧内只执行最后一次页头/页面布局；筛选默认值恢复合并为一次 `Loaded` 调度，并在卸载/Dispatcher 关闭时安全清理待处理标记。
- 不改变现有导航、绑定、命令、侧栏动画、游戏背景开关、列表虚拟化或页面视觉资源；优化目标是减少重复工作和拖拽过程中的布局抖动。

**验证结果：**

- `scripts/validate-source.py`、`check-xaml.ps1`、`git diff --check` 通过；WPF 静态审查 0 error、18 条既有 warning、172 条 info。
- Playnite 定向 26/26 通过；全量 297 通过、57 跳过、0 失败；Release 解决方案构建 0 warning、0 error。
- `.tmp/ui-qa-performance-v1/render-qa-report.txt` 为 `render-qa OK`，覆盖浅色/深色、多尺寸、设置页、列表滚动和 2560→1100→2560 resize transition。
- 未在真实 Playnite 重启后测量宿主级帧率、内存或 DPI 像素效果；RenderHarness 证明的是受控 WPF 布局与主题回归，仍需宿主人工验收。

## 2026-08-25 UI-328 可选游戏背景材质与关闭后的资源回退

**巡检结论：**

- 游戏背景跟随开关此前没有设置入口；如果只在视觉层隐藏图片，后台仍可能继续解码封面，且环境光层可能因为“有过游戏材质”而隐藏主题回退。
- 这是一个同时影响性能、稳定性和主题观感的共享链路问题，适合在 ViewModel、共享资源和环境层一次修正，而不是给单个页面加例外。

**实现内容：**

- `GameSaveCenterSettings` 新增默认开启的 `FollowSelectedGameBackground`，设置页“外观与动态效果”增加“跟随当前游戏背景”开关；旧安装和旧便携设置缺少该字段时保持原行为。
- 关闭开关时立即取消当前封面异步加载、清除已解码 `ImageSource`/采样 Brush，并在重新打开时只对当前选中游戏重新加载；调节动画或毛玻璃强度不会重复解码。
- 共享游戏背景资源现在只有在“开关开启 + 已有采样材质 + 毛玻璃可用”时才显示图片、tint 和 BlurEffect；否则恢复主题中性材质，避免空图片层或残留环境色。
- `AmbientMaterialLayer` 的主题洗色显示条件与游戏材质实际使用条件统一，修复关闭背景跟随后固定主题环境光被误隐藏的边界问题。

**验证结果：**

- `validate-source.py`、`check-xaml.ps1`、`git diff --check` 通过；WPF 静态审查 0 error、18 条既有 warning、172 条 info。
- Release 解决方案构建 0 warning、0 error；Playnite 测试 296 通过、57 跳过、0 失败。
- `.tmp/ui-qa-game-background-v1/render-qa-report.txt` 为 `render-qa OK`，覆盖浅色/深色、多窗口尺寸和 resize transition；已人工查看设置页外观分类离屏截图。
- 未在真实 Playnite 重启后进行本轮宿主逐像素、DPI、Follow/高对比度和实际性能测量；离屏证据不替代宿主验收。
...
- 底部栏同时显示状态、版本说明和产品名，信息重复且状态灯不在右侧；侧栏品牌区的折叠按钮与生产版气泡争抢同一行，用户反馈按钮点击后没有产生有效收起效果。
- Playnite 打开的设置宿主尺寸偏小，导致表单输入区域和右侧内容显得拥挤。

**实现内容：**

- 底部栏改为右对齐的 Worker/Ludusavi 状态区，移除“生产版 · 真实数据由 Worker 提供”和 GameSaveCenter 重复文案。
- 侧栏品牌区恢复生产版气泡位置，气泡显示程序集版本（例如 `v0.6.70`）；折叠按钮移到标题下方独立的 32 DIP 工具条，避免挤压品牌和版本信息，同时保留 78/236 DIP 折叠宽度切换和页面响应式重算。
- 设置 UserControl 增加 `MinWidth=1180`、`MinHeight=760`，只提高 Playnite 默认宿主请求尺寸，不改变设置字段、保存/取消、分类和滚动语义。
- 扩展生产壳源码契约，覆盖右对齐状态、版本气泡、按钮新位置和设置默认尺寸。

**验证结果：**

- `validate-source.py`、`check-xaml.ps1`、`git diff --check` 通过；Release 构建 0 warning、0 error。
- Playnite 测试 295 通过、57 跳过、0 失败；新增/更新生产壳契约 3/3 通过。
- RenderHarness 全量 `render-qa OK`，设置页扩大尺寸后仍通过多尺寸、浅色/深色和 resize transition；真实 Playnite 中的按钮点击与宿主最终窗口尺寸仍需重载扩展后确认。

## 2026-08-25 UI-322 底部状态栏与侧栏折叠

**问题确认：**

- 生产壳侧栏底部已有 Worker/Ludusavi 状态卡，但页面底部栏只有版本说明和产品名，状态信息与底部空间没有形成统一的壳体 chrome。
- 侧栏导航在常规宽度下始终占用完整文字宽度，缺少不改变默认展开布局的紧凑模式入口。

**实现内容：**

- 将 Worker/Ludusavi 两个动态状态灯移入跨越整个壳体底部的 `FooterSurface`；保留健康状态绑定、不可用/正常文案、圆角材质和产品信息，不再在侧栏重复占用状态卡空间。
- 在侧栏品牌区右上角增加 26×26 的共享 WPF-UI 小按钮。默认侧栏仍为 236 DIP 展开宽度；点击后切换到 78 DIP，仅保留导航图标，并同步更新工具提示、自动化名称、箭头字形和页面可用宽度。
- 折叠/展开只改变侧栏 chrome 与导航标签可见性，复用现有 `ApplyHeaderLayout`、`ApplyPageLayout` 响应式计算，不改变页面绑定、命令、滚动、虚拟化或工作区导航。
- 为生产壳新增源码契约，锁定底部状态归属、全宽跨列、默认展开宽度、折叠按钮尺寸和展开/收起宽度恢复。

**验证结果：**

- `validate-source.py`、`check-xaml.ps1`、`git diff --check` 通过；Release 构建 0 warning、0 error。
- Playnite 测试 294 通过、57 跳过、0 失败；新增生产壳 chrome 契约 2/2 通过。
- RenderHarness 全量 `render-qa OK`，覆盖多尺寸、浅色/深色主题与 resize transition；该离屏证据不替代真实 Playnite 宿主中折叠按钮点击、DPI 和键盘焦点验收。

## 2026-08-25 UI-321 修复游戏选框平台默认值的显示时序

**问题确认：**

- UI-320 已补齐默认值和平台集合监听，但生产弹层本身初始为 `Collapsed`；第一次打开时 ComboBox 的 ItemsSource 尚未完成生成，过早执行恢复会因为 `Items.Count == 0` 直接返回，导致中间框仍为空。

**实现内容：**

- 生产壳在弹层打开、平台选项集合变化和平台 ComboBox `Loaded` 后，分别排队到 `DataBind` 与 `Loaded` 优先级再次恢复筛选值。
- `UiFilterSelection.RestoreDefault` 现在会把集合重建后残留的无效选中对象视为空选择，但保留用户当前仍有效的其它平台选择。
- 未改变筛选绑定、游戏列表虚拟化、选中游戏命令或状态默认值；中间框的逻辑默认仍是“全部”。

**验证结果：**

- `validate-source.py`、`check-xaml.ps1`、`git diff --check` 通过；Release 构建 0 warning、0 error。
- Playnite 定向测试 292 通过、57 跳过、0 失败；新增源码契约覆盖弹层 `Loaded` 和 `DataBind/Loaded` 恢复路径。
...
- UI-330 已将模糊半径改为与 `GlassEffectStrength` 直接按百分比对应：20%→20 DIP、默认 78%→78 DIP、100%→100 DIP；继续使用性能渲染偏好。无图、关闭毛玻璃和高对比度返回真正的 null。
...
- 生产页的“毛玻璃强度”此前主要影响卡片表面 alpha；强度拉到 100 后，表面反而更接近不透明，顶部固定环境光被阅读表面遮住，所以视觉上只剩顶部两侧较明显。
- 运行时右上/右下环境光的语义色资源虽然存在，但共享环境光层的覆盖范围和强度没有随滑块联动；中心大面积留白也缺少低透明度的材质洗色。

**实现内容：**

- `AdaptiveThemePalette` 新增规范化 `GlassStrength`，让 accent/info/success 三色环境光和新增的中心柔光令牌按 20–100 联动；100 会明显增强固定装饰层，关闭毛玻璃/高对比度仍走不透明降级。
- `AmbientMaterialLayer.xaml` 将固定装饰椭圆从三个扩展为四个，适度扩大四周覆盖并新增低 alpha 中心柔光；Blur 半径由 18 调整为 24，仍只挂在固定尺寸的装饰椭圆上，不进入根页面、TextBlock、DataGrid、ListBox 或滚动器。
- 高强度下共享玻璃表面采用受控的透光曲线，避免卡片在 100% 时成为不透明遮罩；低强度保留稳定阅读底色。没有修改命令、Binding、滚动模型、虚拟化或 Playnite WindowChrome。
- WPF 资源测试增加“100 比 78 环境光更强”的回归断言，并锁定关闭效果时 Blur 仍为真实 null。

**验证结果：**

- `validate-source.py`、XAML 结构检查、`validate_wpf_ui.py` 均通过；WPF 静态审查无 error。
- 临时开启毛玻璃 100% 的主题离屏 QA 通过；恢复 RenderHarness 测试夹具后，标准全量 `render-qa` 仍通过，覆盖浅色/深色、多尺寸、Tab、滚动和 resize transition。
- Release 构建 0 警告/0 错误；Core 59/59、Worker 199/199、Playnite 282/282，57 项按既有环境规则跳过；标准 `render-qa` 为 `render-qa OK`。
- 受控一键安装成功，生产扩展清单 `0.6.70`、DLL `0.6.70.0`；最终真实宿主是否因显卡驱动、Playnite 主题或系统透明度策略产生不同强度，仍需用户在本机观察。
...
- UI-301 将带放大镜的 TaskCenter/Dashboard 搜索框内容起点收紧到 `20` DIP，但该留白不足以同时容纳图标和文字，导致占位提示与放大镜贴合。
- 生产 TextBox 模板的 `PART_ContentHost` 没有显式继承 TextBox 的 `Foreground`，在 Playnite 宿主主题介入时可能出现实际输入文字几乎不可见，而占位提示仍可见的情况。

**实现内容：**

- TaskCenter 的“搜索任务…”和 Dashboard 的游戏库搜索框，将输入内容与提示的左侧起点统一恢复为 `30` DIP；右侧 `38` DIP 清除按钮区、清除逻辑、Binding、命令和键盘焦点保持不变。
- `WpfUiProduction.xaml` 与 `DesignTokens.xaml` 的共享 TextBox 模板均把 `TextElement.Foreground` 明确传给 `PART_ContentHost`，修复真实输入文字的前景色继承；普通输入、数值输入专用对齐和无障碍语义不变。
- 增加对应源码契约测试，避免图标搜索框再次与提示文字重叠，也避免模板回归导致输入文字不可见。

**验证结果：**

- `validate-source.py` 通过；XAML 结构 19/19；WPF 静态审查 0 error、20 warnings、165 info。
- Release 构建 0 warning/0 error；Core 59/59、Worker 199/199、Playnite 282/282，57 项按既有环境规则跳过。
- `artifacts/ui-qa/ui302-task-search-v1/render-qa-report.txt` 为 `render-qa OK`，已抽查深色主题 Task 页面，图标与提示文字保持独立间距。
- 受控一键安装与生产扩展核验成功，扩展清单为 `0.6.70`、DLL 为 `0.6.70.0`；未声称完成真实 Playnite 逐像素输入文字截图。
...
- 任务中心和游戏库搜索框为搜索图标预留了 `30/32` 的左侧内容区，但在 UI-301 的收紧阶段误将输入起点压到 `20`，造成图标与提示文字过近；这不是普通 TextBox 左对齐模板失效。

**实现内容：**

- 共享 `GscRoundedDataGridRowTemplate` 和 Dashboard 本地兼容行模板统一使用 `4,2,12,2`，只扩大右侧安全区，不改 DataGrid 选择、排序、滚动或虚拟化契约。
- TaskCenter、Dashboard 游戏库搜索框的左侧内容留白曾收紧到 `20`，后由 UI-302 修正为带图标字段使用 `30`；搜索图标、占位提示和右侧清除按钮的命中区保持不变；普通输入框、清除逻辑、Binding 和键盘焦点行为不变。
...
- 在跳过真实 Playnite 宿主验收的前提下，继续完善功能稳定性、安全性和性能相关的工程质量。

**实现内容：**

- 保留策略清理现在必须携带已确认预览的 UTC 时间、候选数量和预计释放空间；Worker 会在真正删除前重新计算，预览过期或索引状态变化时拒绝执行，避免确认旧页面后误删新状态。
- 归档删除改为先移动到 `.gsc-retention-quarantine` 隔离目录，再删除 SQLite 索引，索引操作失败时尝试恢复原路径；隔离文件最终删除失败会返回待清理数量和体积，并写入审计日志。
- 预览摘要明确提示候选明细最多展示 200 条，避免用户误以为列表已经覆盖全部候选。
- Playnite 端应用清理请求改为传递预览快照信息；共享契约新增待清理隔离文件统计，不改变已有命令和确认语义。

**验证结果：**

- Worker 定向测试 6/6 通过，覆盖取消、保护项、ZIP 路径限制、确认要求、预览过期和索引状态变化。
- `git diff --check` 通过；隔离构建输出已从各项目 `.tmp/validation` 清理。
...
# 2026-09-12 U12-06 任务诊断复制可用性

- `CopyTaskErrorCommand` 不再只依赖技术详情；失败原因、错误码或技术详情任一存在时即可复制完整诊断载荷。
- 新增定向源码回归测试，防止后续失败卡片的视觉去重意外禁用复制入口。

- 任务 Inspector 已调整为失败卡片优先、技术详情折叠：失败原因/错误码先显示，`TaskTechnicalDetailsExpander` 默认收起；复制命令仍携带完整三类诊断字段和任务 ID。
- 新增失败详情顺序与折叠状态源码回归测试；U12-06 的视觉结构已闭环。
...
- Q25-01 先用基准暴露 2000 项搜索 p95=201ms 超过 100ms 目标，随后修复 `GamePickerViewModel` 的 180ms debounce 和每项诊断列表分配：debounce 改 20ms，热路径改直接谓词，诊断仍按需生成。复测预热 5 次、采样 30 次为 p50/p95/max=54/57/63ms；Q25-01 自动目标通过，宿主页面导航待验。
- Q25-01 的 30 次采样测试现在额外固化 `search_p95_ms <= 100` 回归门禁；它与证据文件的 p50/p95/max=54/57/63ms 同源，不再只依赖人工阅读基准文本。
- `5f60404` 的最新一键门禁验证 XAML 24/24、Release 0 warning/0 error、Core 76/76、Worker 311/311、Playnite 468/525（57 skip，0 fail）；脚本因当前代码/文档未提交而停止打包。此前 `6450f6e` 的干净安装/启动证据继续保留，但不冒充 5f60404 的最新安装。
...
- Q02-02/Q02-03 将存档历史文件数/大小列迁移到共享 Tabular 数字样式，右对齐并禁止数值截断；时间、状态、备注仍保持文本语义，宿主列呈现继续待验。
...
## 2026-09-22 Round3 R23-08 下一轮准入清单
