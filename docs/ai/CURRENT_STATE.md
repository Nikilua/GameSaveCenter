...
- 当前代码基线为 `8493c1a`，已推送 `codex/ui-finesse-round2`。设置页错误详情 Expander 标题已显式使用 `GscPrimaryTextBrush`，修复了页面根前景存在但标题模板仍可能落回暗色默认黑字的真实漏检；随后补齐了精修夹具的提交身份元数据。
...
- 当前代码基线为 `8493c1a`。RenderHarness `finesseprobe` 报告现在自带 `Scenario`、`Commit`、`WorkingTreeClean`、`DpiScale`、主题和数据范围，避免把未绑定 SHA 的历史夹具当作当前证据。
