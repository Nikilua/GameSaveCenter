...
- 真实 Playnite、硬杀/断电窗口、跨进程并发和长时调度压力仍为 `MANUAL QA REQUIRED`；V2-04～V2-07 尚未处理。

## 2026-09-06 复查后待办（V2-01/V2-02/V2-03 已完成，余项尚未修复）
...
- 当前优先源码风险转为云端 check 取消/异常可能遗留 Transferring，以及 IPC 重放内容未绑定、云队列摘要容量和媒体多页累积成本。均需对应行为/故障测试，不代表已经真机复现。
- 覆盖旧 F01/F03 交接中的保证：已实现存储账本不等于所有崩溃窗口闭环；V2-01～V2-03 已补齐对应的服务级提交、调度和候选恢复行为，但仍不能把自动测试写成断电或真实宿主证据。
- 本阶段 Release 无警告/错误，Core 65/65，Worker 285 通过/1 跳过，Playnite 339 通过/62 跳过，XAML 19/19。Worker 重启及 5 项 Named Pipe 行为测试按既有环境规则跳过；真实宿主仍为 MANUAL QA REQUIRED。
