...
- `a799317b` 在 MediaCenter 归类预览与批次历史中接入共享时间合同；`eb6a8682` 继续复用 `TimeDisplayFormatter`，为 `ValidationFindingDto` 增加相对/完整/原始证据时间，并让 Maintenance 两个诊断详情入口显示相对时间、Tooltip/Automation HelpText 保留完整时间；`0db4abe7` 将 `BackupVersionDto.RestoreReadinessCheckedDisplay` 的 SaveCenter 详情入口接入相对/完整时间；`058ca8ff` 将 `LocalMirrorStatusDto.LastSyncDisplay` 的 Maintenance 镜像卡片入口接入相对/完整/原始 UTC 显示；`308c2b06` 将 `RecentAccessItem` 的 Overview 最近访问行接入相对/完整/原始 UTC 显示，`6ea7bf9a` 补齐 `DateTime.MinValue` 未知时间负例，保留旧兼容语义及恢复、同步、导航安全边界。R22 时间 `14/14`，`MaintenanceReportSourceTests | R17FindingTriageBehaviorTests 7/7`，相关 Save/排序/历史导航/最近访问 `30/30`、有限列表/刷新/来源规则 `4/4`；MediaSyncService 分类回归 `19/20`（排除已知 `MediaSyncServiceTests.cs:570` NRE 为 `19/19`）；最终联合回归 `31 passed / 3 failed / 0 skipped`（总计 34，3 条既有 R14/字体断言漂移，未改写）。R15 `3/3`、Overview `1/1`、R10 `2/2`、R06 `4/4`、R11 `3/3`；Retention Playnite `15/15`、Worker `12/12`。当前提交 `d82d39e7` Release Playnite/Playnite Tests `0 errors`（Playnite 主项目仍有 2 条既有 CS8602 warning）；source、XAML `24/24`、diff 和 WPF `0/27/177` 通过。
