...
## 2026-09-06 复查后待办（V2-01～V2-04 已完成，余项尚未修复）
...
- 当前优先源码风险转为云端 check 取消/异常可能遗留 Transferring、云队列摘要容量和媒体多页累积成本。均需对应行为/故障测试，不代表已经真机复现。
- 覆盖旧 F01/F03 交接中的保证：已实现存储账本不等于所有崩溃窗口闭环；V2-01～V2-04 已补齐对应的服务级提交、调度、候选恢复和 IPC 身份行为，但仍不能把自动测试写成断电或真实宿主证据。
