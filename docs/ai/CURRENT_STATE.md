...
## 2026-09-06 复查后待办（V2-01～V2-06 已完成，余项尚未修复）
...
- 当前优先源码风险转为媒体多页累积成本。需要对应行为/性能测试，不代表已经真机复现。
- 覆盖旧 F01/F03 交接中的保证：已实现存储账本不等于所有崩溃窗口闭环；V2-01～V2-06 已补齐对应的服务级提交、调度、候选恢复、IPC 身份、云端校验代际和完整摘要行为，但仍不能把自动测试写成断电或真实宿主证据。
- 本阶段 Release 无警告/错误，Core 65/65，Worker 294 通过/1 跳过，Playnite 339 通过/62 跳过，XAML 19/19。Worker 硬重启及部分 Named Pipe 行为测试按既有环境规则跳过；真实宿主仍为 MANUAL QA REQUIRED。
