...
- 先由 `efa9614f` 收口 SaveCenter 策略控件名称，再由 `74b559e2` 收口任务/远端进度控件，`c327f92a` 收口 TrainerCenter 导入确认控件，当前 `5d9cb81c` 复用 TrainerCenter 工具设置 Binding、禁用条件和 ToggleSwitch 实现，为版本/风险/运行策略选择器与开关补稳定 Automation 名称；没有新增服务、DTO、命令或业务语义。
- `R21AutomationValueBehaviorTests` 当前 `6/6`，相关进度/焦点/键盘/无障碍/生产壳层回归 `54/54`。新增测试实际检查三个选择器、四个 ToggleSwitch 的 WPF peer、Toggle `Off/On` 和版本值变化；既有 R06/R12 行为夹具继续覆盖未知/排队/零值/越界/取消/成功及远端下载、校验、取消/失败边界。
- 当前 source-copy Release Playnite `net462` / Tests `net472` 为 `0 errors`，仅有 `MediaCenterView.xaml.cs:671` 的 2 条既有 `CS8602` warning；source/XAML/diff 通过，WPF 静态检查 `0/27/177`。真实 Playnite/package-host、Windows UIA/读屏、OS 输入、IME、物理 DPI/跨屏、呈现和宿主性能未验；Demo 原目录不可用，main 用户改动未碰、未合并。证据见 `evidence/R21-02-TRAINER-SETTINGS-AUTOMATION-20260921.md`。
