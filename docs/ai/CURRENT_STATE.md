...
- 验证：Release 构建 0 错误（1 个因 nuget.org 漏洞服务不可达产生的 `NU1900` 警告）；全量 Core `72/72`、Worker `300/301`（1 跳过）、Playnite `363/425`（62 跳过）；源码校验、XAML `19/19`、`git diff --check` 通过。真实 Playnite、DPI/高对比度、完整键盘和 60fps 流畅度仍需外部验收。
