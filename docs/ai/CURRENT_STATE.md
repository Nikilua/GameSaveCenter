...
- 待归类页在页面宿主高度低于 620 DIP 时隐藏与上方指标重复的标题/数量信息带，将空间交还给批量工具栏和有限 DataGrid；正常高度保持原信息带。
- 整页 fallback scroll 阈值由 620 收紧到 520 DIP；离线/过期横幅仍强制安全降级，避免表格视口归零。批量操作、Inspector、选择、虚拟化和末页入口未改。
- Release Playnite 构建通过（0 warning / 0 error）；真实宿主的 1040×700/1100×720、物理滚动末端和 DPI 仍待验收。
