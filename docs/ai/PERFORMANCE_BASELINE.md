...
## L22 缩略图边界基线（2026-09-09）

- RenderHarness `thumbnailprobe` 使用 120 个 96×96 合成 PNG 和一个隐藏 STA WPF Window；初次窗口 `120` 请求/`120` 成功、峰值并发 `3`、缓存 `96/96`、活动解码归零。16 项保留窗口产生 `16` 次缓存命中且没有新增解码。
- 破损 PNG 与缺失 PNG 都返回空占位并可删除；预取消请求被计为取消；100 次 12 项窗口往返请求增量 `1200`、缓存命中增量 `1089`、解码增量 `111`，峰值并发仍为 `3`、活动解码为 `0`、缓存不超过 `96`。800×800 旧图立即替换为 64×64 新图后，控件最终得到宽度 `96` 的新图标记。
- 这是缓存/并发/生命周期的可重复边界证据，不是缩略图解码耗时、真实滚动帧率、Playnite/FusionX 模板链、用户目录锁定或 DPI 证据；真实宿主仍为 `MANUAL QA REQUIRED`。报告为 `.tmp/l22-thumbnailprobe-final/thumbnailprobe-report.txt`，不进入 Git。

## E01 隔离规模基线（2026-09-05）

- 入口为 `scripts/e01-scale-baseline.ps1`。full 使用 2,000 游戏、20,000 备份、10,000 任务、5,000 媒体、500 工具，seed `105999 ms`、20 轮 Worker 读/事件/原子写/锁模拟 `427 ms`；游戏查询首查/热查 `17/14 ms`，任务页首查/热查/下一页 `92/3/3 ms`，媒体页 `3/1/0 ms`，查询/模拟分配 `6635504/61057744` bytes。
- stress 使用 10,000 游戏、20,000 备份、10,000 任务、50,000 媒体、500 工具，seed `242832 ms`、模拟 `1529 ms`；游戏查询首查/热查 `83/84 ms`，任务页 `93/3/2 ms`，媒体页 `4/0/1 ms`，查询/模拟分配 `30162656/294692392` bytes。两档任务和媒体搜索均返回 1 条命中，首屏页均返回 200 条并成功取得下一页。
...
- 该夹具覆盖 Worker/SQLite 数据量、查询首查/热查、搜索/分页、事件/原子写/操作锁循环和资源增长，不覆盖真实 Playnite 冷/热首屏、UI 分配、帧间隔、DPI 或用户目录性能；这些继续为 `MANUAL QA REQUIRED`。
