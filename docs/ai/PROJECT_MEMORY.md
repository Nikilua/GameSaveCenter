...
- `e4389a0904ec5e027a5c04525e18d9c1126209b1` 先接入 `MediaItemDto` 时间，`a799317b` 再将 `MediaClassificationSuggestionDto`、预览和批次历史的 Captured/Created/Updated/Expires 入口接入共享 formatter；`eb6a8682` 将 `ValidationFindingDto` 的证据时间接入相对/完整/原始合同；`0db4abe7` 将 `BackupVersionDto.RestoreReadinessCheckedDisplay` 的 SaveCenter 入口接入相对/完整时间；`058ca8ff` 将 `LocalMirrorStatusDto.LastSyncDisplay` 接入相对/完整/原始合同；`308c2b06` 又将 `RecentAccessItem` 的 Overview 最近访问行接入相对/完整/原始合同，`6ea7bf9a` 补齐 `DateTime.MinValue` 未知负例。`44023ad6` 继续复用 `HealthInspectionStateDto` 和 `TimeDisplayFormatter`，为恢复巡检最近成功/完成/下次计划增加相对、完整、原始 UTC 投影；Maintenance 卡片切换到相对时间，完整时间进入 Tooltip/Automation HelpText，未知、停用、取消、预算、命令和旧兼容路径保持。`0df6c69e` 又为 `EnvironmentCheckReportDto.CheckedUtc` 增加相对、完整、原始 UTC 投影；首次环境检查卡片正文使用相对时间，完整值进入 Tooltip/Automation HelpText，已有检查服务、完成/跳过、测试备份和命令门控不变。`6a27ef42` 又为 `CloudTransferStatusDto` 的最后尝试与最后成功校验增加相对、完整、原始 UTC 投影；选中云端详情正文显示相对时间，完整值进入 Tooltip/Automation HelpText，队列分页、筛选、重试/校验、脱敏和有限列表不变。最终 `R22TimeDisplayBehaviorTests 17/17`，云端/Health/Environment/时间/维护定向 `39/39`，Worker `EnvironmentCheckServiceTests 1/1`，`MaintenanceReportSourceTests | R17FindingTriageBehaviorTests 7/7`，Save/排序/历史导航/最近访问相关 `30/30`，有限列表/刷新/来源规则 `4/4`，Worker 分类 `19/20`（排除已知 `MediaSyncServiceTests.cs:570` NRE 为 `19/19`），联合回归 `34 passed / 3 failed / 0 skipped`（总计 37，3 条为既有 R14/字体漂移）；当前代码提交 `6a27ef42` 的 Release 隔离构建 XAML `24/24`、Playnite/Tests `0 errors`，保留 2 条既有 MediaCenter CS8602，WPF 静态 `0/27/177`，source/diff 通过。
