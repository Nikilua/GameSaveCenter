...
- `e4389a0904ec5e027a5c04525e18d9c1126209b1` 先接入 `MediaItemDto` 时间，`a799317b` 再将 `MediaClassificationSuggestionDto`、预览和批次历史的 Captured/Created/Updated/Expires 入口接入共享 formatter；保留旧 Local/DetailDisplay 属性以及分类预览的选择、应用、撤销、过期和恢复语义。最终 `R22TimeDisplayBehaviorTests 10/10`，相关有限列表/刷新/来源规则 `4/4`，Worker 分类 `19/20`（排除已知 `MediaSyncServiceTests.cs:570` NRE 为 `19/19`），组合 `27 passed / 3 failed / 0 skipped`，3 条为既有 R14/字体漂移；Playnite/Tests Release `0 errors`，WPF 静态 `0/27/177`。
- R22-01 尚未完成：`ValidationFindingDto.EvidenceTimeDisplay` 及其他 Save/恢复时间入口仍需逐项核对；真实系统时钟跳变/跨系统启动周期、剪贴板、Playnite/package-host、UIA/读屏、呈现、DPI/跨屏、ETW 和宿主性能仍未验。WPF 全套基线的 3 条无关失败未改写为绿色，Demo 原目录不可用，main 用户改动未碰未合并。证据：`R22-01-MEDIA-TIME-20260921.md`、`R22-01-RETENTION-TIME-20260921.md`、`R22-01-SAVE-MAINTENANCE-TIME-20260921.md`、`R22-01-OVERVIEW-TIME-20260921.md`、`R22-01-TIME-DISPLAY-20260921.md`、`R22-01-MONOTONIC-TASK-DURATION-20260921.md`。下一步处理 Maintenance 诊断证据时间和未知时间负例。
