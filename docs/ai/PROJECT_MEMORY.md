...
- `e4389a0904ec5e027a5c04525e18d9c1126209b1` 先接入 `MediaItemDto` 时间，`a799317b` 再将 `MediaClassificationSuggestionDto`、预览和批次历史的 Captured/Created/Updated/Expires 入口接入共享 formatter；`eb6a8682` 将 `ValidationFindingDto` 的证据时间接入相对/完整/原始合同；`0db4abe7` 将 `BackupVersionDto.RestoreReadinessCheckedDisplay` 的 SaveCenter 入口接入相对/完整时间；`058ca8ff` 又将 `LocalMirrorStatusDto.LastSyncDisplay` 接入相对/完整/原始合同，保留旧 stale/unknown/未同步语义。最终 `R22TimeDisplayBehaviorTests 13/13`，`MaintenanceReportSourceTests | R17FindingTriageBehaviorTests 7/7`，Save/排序/历史导航相关 `27/27`，有限列表/刷新/来源规则 `4/4`，Worker 分类 `19/20`（排除已知 `MediaSyncServiceTests.cs:570` NRE 为 `19/19`），组合 `28 passed / 3 failed / 0 skipped`，3 条为既有 R14/字体漂移；精确 `058ca8ff` Release Playnite/Tests `0 errors`，Playnite 主项目保留 2 条既有 CS8602 warning，WPF 静态 `0/27/177`。
- R22-01 尚未完成：还需逐项核对 Storage/RecentAccess 等其他 Save/恢复或维护时间直显、兼容属性和未知时间负例；真实系统时钟跳变/跨系统启动周期、剪贴板、Playnite/package-host、UIA/读屏、呈现、DPI/跨屏、ETW 和宿主性能仍未验。联合门禁的 3 条无关基线失败未改写为绿色，Demo 原目录不可用，main 用户改动未碰未合并。证据：`R22-01-MEDIA-TIME-20260921.md`、`R22-01-RETENTION-TIME-20260921.md`、`R22-01-SAVE-MAINTENANCE-TIME-20260921.md`、`R22-01-OVERVIEW-TIME-20260921.md`、`R22-01-TIME-DISPLAY-20260921.md`、`R22-01-MONOTONIC-TASK-DURATION-20260921.md`。下一步盘点 Storage/RecentAccess 和其他 Save/恢复时间入口。
