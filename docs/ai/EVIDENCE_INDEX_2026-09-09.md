...
当前基线：`main`，文档基线提交 `d439140`；候选包内生产代码提交为 `8d729ab`。本索引只汇总已有证据，不把离线夹具或源码断言扩大为真实宿主结论。
...
| 任务/媒体离线滚动 | `.tmp/l32-scrollprobe/scaleprobe-report.txt` | 200/2000/10000 条、20 次往返、滚轮、PageUp/PageDown、Ctrl+End；`scaleprobe OK`，未发现空正文/选框分离；末尾滑块路径最后行完整 | `ScrollIntoView(最后一项)` 在插件/标准离屏模板均为 `offscreen-inconclusive` |
...
- 离线规模探针：使用 RenderHarness `scaleprobe`，输出应保留在 `.tmp/l32-scrollprobe/scaleprobe-report.txt`。
