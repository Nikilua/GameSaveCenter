...
当前基线：`main`，窗口级滚动探针提交为 `85b1aeb`；候选包内生产代码提交为 `8d729ab`。本索引只汇总已有证据，不把离线夹具或源码断言扩大为真实宿主结论。
...
| 任务/媒体离线滚动 | `.tmp/l32-scrollprobe/scaleprobe-report.txt` | 200/2000/10000 条、20 次往返、滚轮、PageUp/PageDown、Ctrl+End；`scaleprobe OK`，未发现空正文/选框分离；末尾滑块路径最后行完整 | 隐藏 WPF `Window` 同时对照插件、标准 WPF 和本机已安装 FusionX `2.1.1` 的 `DefaultControls/DataGrid.xaml`；FusionX 直接滑块/Ctrl+End 为 `1987/1987`、末行完整，deferred `ScrollIntoView` 仍基线不确定；均不能替代真实 Playnite |
