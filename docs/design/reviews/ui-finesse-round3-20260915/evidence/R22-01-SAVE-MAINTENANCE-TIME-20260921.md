...
提交 `2be8627db39304a2a3f929a797799c5c366b28c0`、`c211a04f5c28b400bac976cfc2223cae2a709299`、`0db4abe7`、`058ca8ff`、`44023ad6`、`0df6c69e`、`6a27ef42` 和 `f1e568d0` 完成 Save 历史/详情、Maintenance 审计表、恢复可用性检查时间、Local Mirror 最近同步时间、恢复巡检时间、首次环境检查时间、云端队列选中详情证据时间及下次重试相对时间的显示接入。R22-01 整项仍为“部分满足，待继续”，保留其他时间入口和真实宿主边界，未修改真实存档、诊断写入或用户目录。
...
- `CloudTransferStatusDto` 的 `RetryTimingDisplay` 兼容属性保留；新增相对/完整/原始下次重试时间投影，选中队列详情显示相对状态，完整下次时间进入 Tooltip/UIA HelpText。未来重试、到期可立即重试、无自动重试的状态文案保持。

## 实际验证

- `R22TimeDisplayBehaviorTests` `18/18`：共享 formatter、任务/活动、BackupVersion/AuditLog DTO、恢复校验、Local Mirror、HealthInspection、EnvironmentCheck、CloudTransfer 详情及 RetryTiming 时间的完整/原始 UTC 合同，以及 Save/Maintenance 绑定入口；云端/Health/Environment/时间/维护定向 `40/40`；Worker `EnvironmentCheckServiceTests 1/1`；`MaintenanceReportSourceTests | R17FindingTriageBehaviorTests` `7/7`。
...
继续 R22-01：先核对 Maintenance 动作摘要中 CloudTransfer 的 `LastAttemptDisplay`/`NextAttemptDisplay`，再盘点 Storage，复用已有 formatter 后补未知/兼容负例；再处理系统时钟和宿主边界。
