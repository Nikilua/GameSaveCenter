...
- 当前 `b5c7a6d423a4bf23004c3b080e133b3b0b065fa5` 的 `UiFinesseFoundationTests` 为 `9/9`，实际覆盖完成态、清钟、当前 Dispatcher 值重入和 reduced-motion 取消；行为测试未依赖源码字符串。
- 同身份的额外 `motionreentryprobe`/`motionhotprobe` 在本机重复运行未稳定通过，失败点分别为 Light 最终动画标志未及时归一、Dark 活动态采样未达到门槛。该结果不被写成动画行为通过，也没有据此扩大生产改动；本证据项的可控行为结论与 probe 时序边界分开记录。
