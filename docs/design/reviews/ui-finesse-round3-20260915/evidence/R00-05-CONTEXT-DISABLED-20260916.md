...
- 首次直接使用默认 `bin\Release` 的复跑被 R01-01 身份门禁拒绝：程序集仍来自 `447ac07e`，源码根已是当前提交，未把这次拒绝计为产品失败，也未绕过校验。
- 从当前 checkout 的隔离 OutputRoot `.tmp/r00-05-06-build-clean` 重建后，R00-05 的 Light/Dark 实际 WPF 派生样式测试 `2/2` 通过；Context/RemoteRestore/MediaBatch 均观测到控件 `Opacity=1`、模板 `ButtonChrome=0.72`，启用/禁用高度无跳变，双主题合成最低上下文文本对比度保持 `3.0` 门槛。
- 当前构建为 clean-tree `e8fe1aab`，没有改生产样式或命令/绑定；本证据仍只覆盖受控 WPF 主题资源、合成内容和 offscreen logical DIP，不等价真实宿主像素、用户主题、物理 DPI 或真实输入。
