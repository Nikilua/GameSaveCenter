...
采集日期：2026-09-15（Asia/Shanghai）。当前代码基线：`39e37b1`。证据来自共享 XAML/C# 源码、Playnite 设置迁移测试、RenderHarness Offscreen Regression Audit、受控 STA WPF Window 运行时测试、隔离 Playnite 真实宿主加载日志和已提交 HEAD 的打包/安装记录；本次已取得 `EmbeddedPlaynite` Dashboard 生产像素，并修正 150% 宿主截图的 DPI 叠加，补齐 Overview 空活动/多风险/超长标题/离线边界夹具和 Q25-03 受控 UI 动作耗时边界，新增六态生产状态夹具、重入动画接管、完成态时钟释放和生产侧栏窗口关闭卸载清理门禁；离屏/受控证据仍不冒充 IME、物理跨屏、读屏、ETW 帧时间或停顿期间调用栈。
...
| Q25 | 大库批量更新、缩略图并发/缓存边界、审计性能字段、构建身份、打包/安装验证 | `LargeLibraryPerformanceTests`、`AsyncThumbnailLoaderTests`、`DiagnosticsEvidenceSourceTests`、`BuildIdentityTests`；`Q25-01-HOT-RESPONSE-20260915.md` 记录当前干净提交 5 次预热、30 次输入到反馈采样及 p50/p95/max=45/46/60 ms，`Q25-02-RENDERING-PROXY-20260915.md` 记录当前干净提交 WPF Rendering 回调的 p95/最大间隔/60Hz 慢帧比例代理，`Q25-03-UI-HOTSPOT-20260915.md` 记录当前干净提交受控 WPF 动作周期 p95/max=`249.94/483.44 ms`、`>100ms`=`254/254` 及完成后栈边界，`Q25-04-ENDURANCE-20260915.md` 记录当前干净提交受控 WPF Window 的 1800.4 秒/3847 循环/13462 动作/176 资源样本，`Q25-05-LOW-COST-FALLBACK-20260915.md` 记录当前干净提交双主题六工作区禁玻璃/禁动画 24/24 视觉回退，`Q25-08-RELEASE-TEST-BASELINE-20260915.md` 同时记录 `d8fad48` 历史基线、`c76ce62`/`4414f05` 动效跟进、`68b49a1` 通用动效回归和 `39e37b1` 生产侧栏卸载回归，当前为 Core 83/83、Worker 310/311（1 skip）、Playnite 487/550（63 skip）；`Q25-PERFORMANCE-BENCHMARK-20260914.txt` 保留前一轮基线；`REAL_HOST_AUDIT-20260914.md` 记录 `4f1dbb4` 非空隔离包体、主题复制、宿主原生命令、27/2/1 嵌入捕获、DPI 截图修正、安装和 Playnite 加载；`Q25-ETW-TOOL-BOUNDARY-20260914.md` 记录 xperf DWM 会话被 `0x5 / Access denied` 拒绝；最新源门禁为 XAML 24/24、Release 0 warning/0 error、Core 83/83、Worker 310/311（1 skip）、Playnite 487/550（63 skip） | 受控代理、UI 动作边界、30 分钟窗口和动效终态释放已有证据，但真实屏幕呈现帧、Playnite 宿主耐久、停顿期间 `>100ms` 调用栈和低性能 Tier 尚未签收 |

> Q25-08 最新跟进：`39e37b1` 的 Release 全量为 Core `83/83`、Worker `310/311`（1 skip）、Playnite `487/550`（63 skip），失败 `0`；新增生产侧栏 STA WPF Window 完成/窗口关闭卸载回归并修正位移归零，详细历史与动效清理跟进见 [`Q25-08-RELEASE-TEST-BASELINE-20260915.md`](q13-q25/Q25-08-RELEASE-TEST-BASELINE-20260915.md)。
