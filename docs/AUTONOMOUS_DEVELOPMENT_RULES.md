...
- UI 改动必须以 AcrylicFork Demo 为最高视觉基准，并阅读 `docs/design/APPLE_WPF_IMPLEMENTATION_PROMPT.md` 和 `docs/design/UI_CHANGE_GATE.md`。`wpf-apple-desktop-ui` Skill 仅用于质量检查，不得限制或改变 Demo-first 的页面布局、控件和视觉迁移；仍不得无意删除真实命令、取消、错误或可扩展列表行为；如果新设计有意改变这些能力，必须明确迁移并配套测试。
