...
namespace GameSaveCenter.Playnite.Tests
{
    public sealed class UiLayoutRegressionTests
    {
        ...
        [Fact]
        public void OverviewTodayUsesTheSharedCardSurfaceWithoutNestedWash()
        {
            var root = FindRepositoryRoot();
            var overview = XDocument.Parse(File.ReadAllText(Path.Combine(root, "src", "GameSaveCenter.Playnite", "Views", "OverviewView.xaml")));
            var xamlName = XName.Get("Name", "http://schemas.microsoft.com/winfx/2006/xaml");
            ...
            Assert.Equal("{DynamicResource GscRedesignSectionCard}", hero.Attribute("Style")?.Value);
            Assert.Null(hero.Attribute("ClipToBounds"));
        ...
        public void SaveHistorySizeUsesNoTrimSemanticStyle()
        {
            ...
            var sizeColumn = document.Descendants().Single(element =>
                string.Equals(element.Name.LocalName, "DataGridTextColumn", StringComparison.Ordinal)
