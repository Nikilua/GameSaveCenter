...
<UserControl xmlns=""http://schemas.microsoft.com/winfx/2006/xaml/presentation""
    ...
    public void TaskGridKeepsBottomRowAboveScrollbarWithoutChangingColumnScrollContract()
    {
        ...
        Assert.DoesNotContain("<Setter Property=\"infra:DataGridStarFill.Enabled\" Value=\"False\"/>", task);
        Assert.Contains("FakeDashboardData(500)", harness);
        Assert.Contains("gridName is \"MediaInboxGrid\" or \"TaskGrid\"", harness);
        Assert.Contains("bottom last row is under horizontal scrollbar", harness);
    }
    ...
    public void LargeGameLibrariesUseOneVirtualizedSearchableSelectorSurface()
    {
        ...
        Assert.Equal("30,7,38,7", gameSearch.Attribute("Padding")?.Value);
        var gameSearchHint = xaml.Descendants().Single(element => element.Name.LocalName == "TextBlock"
            && element.Attribute("Text")?.Value == "搜索游戏…");
        Assert.Equal("30,0,38,0", gameSearchHint.Attribute("Margin")?.Value);
    ...
    public void PlayniteShutdownStopsQueuedCallbacksBeforeTheyTouchWorkerOrUi()
    {
        ...
        Assert.Contains("WorkerIpcCancellationReason.HostShutdown", pluginCode);
        Assert.Contains("lifetimeCancellation.Token);", pluginCode);
    ...
    public void SettingsRedesignMovesCategoriesWithoutRemovingExistingFieldsOrSaveSemantics()
    {
        ...
        Assert.Contains("Content=\"常规与目录\"", settings);
        ...
        Assert.Contains("SettingsCompactContentRow.Height = new GridLength(1, GridUnitType.Star)", settingsCode);
