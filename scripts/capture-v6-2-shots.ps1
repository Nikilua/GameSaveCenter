...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = Join-Path $root 'artifacts\ui-qa\v6-2-shots'
}
...
try {
    ...
    Write-Host "==> Rendering v6.2 screenshots to $Output" -ForegroundColor Cyan
    & $exe v6-2shots $Output
    if ($LASTEXITCODE -ne 0) { throw "v6.2 screenshot capture failed: $LASTEXITCODE" }
    Write-Host "Report: $(Join-Path $Output 'v6-2-shots-report.txt')" -ForegroundColor Green
}
finally {
    Pop-Location
}
