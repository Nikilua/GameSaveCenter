...
try {
    ...
    $report = @(
        ...
        "- query allocated: $($metrics.queryAllocatedBytes) bytes",
        "- simulation allocated: $($metrics.simulationAllocatedBytes) bytes",
        "- games query first/warm: $($metrics.firstGamesQueryMilliseconds)/$($metrics.warmGamesQueryMilliseconds) ms",
        "- task page first/warm/next: $($metrics.firstTaskPageQueryMilliseconds)/$($metrics.warmTaskPageQueryMilliseconds)/$($metrics.nextTaskPageQueryMilliseconds) ms",
        "- task search: $($metrics.taskSearchQueryMilliseconds) ms ($($metrics.taskSearchResultCount) result)",
        "- media page first/warm/next: $($metrics.firstMediaPageQueryMilliseconds)/$($metrics.warmMediaPageQueryMilliseconds)/$($metrics.nextMediaPageQueryMilliseconds) ms",
        "- media search: $($metrics.mediaSearchQueryMilliseconds) ms ($($metrics.mediaSearchResultCount) result)",
