...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = Join-Path $root 'artifacts\ui-qa\v4-shots'
}
...
try {
    ...
    Write-Host "==> Rendering v4 screenshots to $Output" -ForegroundColor Cyan
    & $exe v4shots $Output
    if ($LASTEXITCODE -ne 0) { throw "v4 screenshot capture failed: $LASTEXITCODE" }
    Write-Host "Report: $(Join-Path $Output 'v4-shots-report.txt')" -ForegroundColor Green
}
finally {
    Pop-Location
}
