...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = Join-Path $root 'artifacts\ui-qa\v3-shots'
}
...
try {
    ...
    Write-Host "==> Rendering v3 screenshots to $Output" -ForegroundColor Cyan
    & $exe v3shots $Output
    if ($LASTEXITCODE -ne 0) { throw "v3 screenshot capture failed: $LASTEXITCODE" }
    Write-Host "Report: $(Join-Path $Output 'v3-shots-report.txt')" -ForegroundColor Green
}
finally {
    Pop-Location
}
