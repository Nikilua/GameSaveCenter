...
WHERE COALESCE(classification_state,'')='' OR classification_state='Assigned';";
        ...
        var mediaGameStateIndex = connection.CreateCommand();
        mediaGameStateIndex.CommandText = "CREATE INDEX IF NOT EXISTS ix_media_game_state_capture ON media(playnite_id,classification_state,captured_utc DESC,media_id DESC);";
        await mediaGameStateIndex.ExecuteNonQueryAsync(token).ConfigureAwait(false);
        var mediaStateCaptureIndex = connection.CreateCommand();
        mediaStateCaptureIndex.CommandText = "CREATE INDEX IF NOT EXISTS ix_media_state_capture ON media(classification_state,captured_utc DESC,media_id DESC);";
        await mediaStateCaptureIndex.ExecuteNonQueryAsync(token).ConfigureAwait(false);
...
INSERT INTO tasks(task_id,request_id,session_id,worker_session_id,task_type,game_id,game_name,state,progress,message,stage_message,created_utc,started_utc,finished_utc,error_code,error_message,restore_report_json)
VALUES($id,$request,$session,$worker,$type,$game,$name,$state,$progress,$message,$stageMessage,$created,$started,$finished,$errorCode,$errorMessage,$restoreReport)
ON CONFLICT(task_id) DO UPDATE SET state=excluded.state,progress=excluded.progress,message=excluded.message,stage_message=excluded.stage_message,worker_session_id=excluded.worker_session_id,
 request_id=excluded.request_id,started_utc=excluded.started_utc,finished_utc=excluded.finished_utc,error_code=excluded.error_code,error_message=excluded.error_message,restore_report_json=excluded.restore_report_json;",
        new Dictionary<string, object?>
        {
            ...
            ["$state"] = (int)task.State, ["$progress"] = task.ProgressPercent, ["$message"] = task.Message, ["$stageMessage"] = string.IsNullOrWhiteSpace(task.StageMessage) ? task.Message : task.StageMessage,
    ...
    public async Task<List<TaskStatusDto>> GetRecentTasksAsync(int limit, CancellationToken token)
    {
        ...
        await connection.OpenAsync(token).ConfigureAwait(false);
        var command = connection.CreateCommand();
        command.CommandText = "SELECT task_id,request_id,session_id,worker_session_id,task_type,game_id,game_name,state,progress,message,stage_message,created_utc,started_utc,finished_utc,error_code,error_message,restore_report_json FROM tasks ORDER BY created_utc DESC, task_id DESC LIMIT $limit;";
        ...
        while (await reader.ReadAsync(token).ConfigureAwait(false))
        {
            result.Add(new TaskStatusDto
            {
                ...
                Message=reader.IsDBNull(9)?string.Empty:reader.GetString(9), StageMessage=reader.IsDBNull(10)?string.Empty:reader.GetString(10), CreatedUtc=DateTime.Parse(reader.GetString(11)).ToUniversalTime(),
                StartedUtc=reader.IsDBNull(12)?null:DateTime.Parse(reader.GetString(12)).ToUniversalTime(),
                FinishedUtc=reader.IsDBNull(13)?null:DateTime.Parse(reader.GetString(13)).ToUniversalTime(),
                ErrorCode=reader.IsDBNull(14)?string.Empty:reader.GetString(14), ErrorMessage=reader.IsDBNull(15)?string.Empty:reader.GetString(15),
                RestoreReport=reader.IsDBNull(16) || string.IsNullOrWhiteSpace(reader.GetString(16)) ? null : JsonSerializer.Deserialize<RestoreReportDto>(reader.GetString(16), _json)
            });
        }
        return result;
    }
...
CREATE TABLE IF NOT EXISTS tasks(task_id TEXT PRIMARY KEY,request_id TEXT NOT NULL DEFAULT '',session_id TEXT NOT NULL DEFAULT '',worker_session_id TEXT NOT NULL DEFAULT '',task_type TEXT NOT NULL,game_id TEXT,game_name TEXT,state INTEGER NOT NULL,progress INTEGER NOT NULL,message TEXT,stage_message TEXT NOT NULL DEFAULT '',created_utc TEXT NOT NULL,started_utc TEXT,finished_utc TEXT,error_code TEXT,error_message TEXT,restore_report_json TEXT NOT NULL DEFAULT '');
...
CREATE TABLE IF NOT EXISTS media_classification_batches(batch_id TEXT PRIMARY KEY,state TEXT NOT NULL,created_utc TEXT NOT NULL,updated_utc TEXT NOT NULL,expires_utc TEXT NOT NULL,last_error TEXT NOT NULL DEFAULT '');
CREATE TABLE IF NOT EXISTS media_classification_batch_items(batch_id TEXT NOT NULL REFERENCES media_classification_batches(batch_id) ON DELETE CASCADE,media_id TEXT NOT NULL,original_playnite_id TEXT NOT NULL,original_classification_state TEXT NOT NULL,original_classification_reason TEXT NOT NULL,original_archive_path TEXT NOT NULL,original_path TEXT NOT NULL,original_captured_utc TEXT NOT NULL,original_size_bytes INTEGER NOT NULL,original_sha256 TEXT NOT NULL,original_is_favorite INTEGER NOT NULL,original_comment TEXT NOT NULL,original_cloud_state TEXT NOT NULL,target_playnite_id TEXT NOT NULL,target_reason TEXT NOT NULL,confidence TEXT NOT NULL,item_state TEXT NOT NULL,applied_archive_path TEXT NOT NULL,updated_utc TEXT NOT NULL,PRIMARY KEY(batch_id,media_id));
CREATE TABLE IF NOT EXISTS media_classification_operations(operation_id TEXT PRIMARY KEY,batch_id TEXT NOT NULL,media_id TEXT NOT NULL,operation_kind TEXT NOT NULL,state TEXT NOT NULL,source_path TEXT NOT NULL,destination_path TEXT NOT NULL,expected_sha256 TEXT NOT NULL,source_is_original INTEGER NOT NULL,destination_preexisted INTEGER NOT NULL,created_utc TEXT NOT NULL,updated_utc TEXT NOT NULL,last_error TEXT NOT NULL DEFAULT '');
CREATE INDEX IF NOT EXISTS ix_media_batch_classification_state ON media_classification_batches(state,updated_utc DESC);
CREATE INDEX IF NOT EXISTS ix_media_batch_classification_item ON media_classification_batch_items(media_id,updated_utc DESC);
CREATE UNIQUE INDEX IF NOT EXISTS ux_media_classification_active_operation ON media_classification_operations(media_id) WHERE state IN ('Planned','Moved','RecoveryRequired');
