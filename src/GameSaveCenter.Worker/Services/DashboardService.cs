...
public sealed class DashboardService
{
    ...
    public async Task<DashboardSnapshotDto> GetAsync(CancellationToken token)
    {
        ...
        {
            GeneratedUtc=DateTime.UtcNow,WorkerHealthy=true,SafeModeEnabled=_options.SafeModeEnabled,WorkerVersion=typeof(DashboardService).Assembly.GetName().Version?.ToString()??"dev",WorkerBuildIdentity=BuildIdentity.ForAssembly(typeof(DashboardService).Assembly),
