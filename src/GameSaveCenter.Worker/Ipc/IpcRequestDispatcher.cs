...
public sealed class IpcRequestDispatcher
{
    ...
    public async Task<IpcEnvelope> DispatchAsync(IpcEnvelope request,CancellationToken token)
    {
        ...
        try
        {
            object payload=request.Type switch
            {
                ...
                MessageTypes.ListUnassignedMediaPage=>await _store.GetUnassignedMediaPageAsync(Read<MediaQueryDto>(request),token).ConfigureAwait(false),
                MessageTypes.ListIgnoredMedia=>await ListIgnoredMediaAsync(Read<GameQueryDto>(request),token).ConfigureAwait(false),
                MessageTypes.ListIgnoredMediaPage=>await _store.GetIgnoredMediaPageAsync(Read<MediaQueryDto>(request),token).ConfigureAwait(false),
                ...
                MessageTypes.IgnoreMediaBatch=>await _media.IgnoreBatchAsync(Read<MediaInboxBatchRequestDto>(request),token).ConfigureAwait(false),
                MessageTypes.RestoreIgnoredMediaBatch=>await _media.RestoreIgnoredBatchAsync(Read<MediaInboxBatchRequestDto>(request),token).ConfigureAwait(false),
    ...
    private WorkerSettingsSnapshotDto SanitizedSettings()=>new()
    {
        ...
        EnableLocalMirror=_options.EnableLocalMirror,
        LocalMirrorPath=_options.LocalMirrorPath,
